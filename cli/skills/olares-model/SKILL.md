...
# model (Router and Model Console)
...
Use `olares-cli model <verb> --help` for authoritative syntax.
...
All verbs require Olares 1.12.6+, the oldest line whose Market carries Router. Router is an admin-only application: a non-admin profile cannot see its entrance, so every verb here reports it is not installed. Confirm with `model status` before concluding anything is missing.

## Verb index

| Family | Verbs | Read when triggered |
|---|---|---|
| where Router is, and who you are | `status`, `whoami` | [architecture and identity](references/olares-model-architecture.md) |
| cloud vendors and their models | `provider list/get/types/create/update/delete/validate/credentials/history/rollback/sync-models`, `provider models import/add/update/delete` | [configuring an external provider](references/olares-model-external.md) |
| local LLM applications | `app catalog/install/upgrade/uninstall/tasks/watch`, `provider register`, `local status/progress/spec/retry/restart` | [local LLM applications](references/olares-model-local-llm.md) |
| local embedding, audio, OCR, CLIP | the same verbs, different modes | [local multimodal applications](references/olares-model-local-multimodal.md) |
| what is configured | `list`, `capabilities`, `default show/set/clear` | [defaults and access control](references/olares-model-governance.md) |
| access control | `key issue/list/update/revoke/local`, `quota set/list/clear`, `caller list/archive`, `user list` | [defaults and access control](references/olares-model-governance.md) |
| what happened | `usage summary/list/export`, `audit list/get`, `trace list/get/capture` | [usage, audit and traces](references/olares-model-usage.md) |
| calling a model | `call chat/embed/transcribe/speak/ocr` | [calling a model](references/olares-model-calling.md) |
| inside one application | `local status/progress/spec/config/endpoints/gpu/perf/retry/restart` | [the Model Console](references/olares-model-console.md) |
| a call or a model that does not work | any of the above | [deciding which layer is wrong](references/olares-model-diagnosis.md) |

## Two planes, two credentials

Read [architecture and identity](references/olares-model-architecture.md) before the first write. In short:
...
- **Calling** (`model call`) needs a data-plane credential of its own. The CLI tries the platform's own identity first and mints an `sk-` key only if that is refused, keeping it in the keychain; `model key local` shows or forgets it.
- Most of the management plane is admin-only, reads included: providers, the vendor catalog's models, `capabilities`, market installs, quotas, users, callers and audit all refuse a non-admin. What a non-admin can do is `model list`, `default show`, `default set --mine`, their own keys, their own usage, their own traces, and `model call`.
...
- A model is addressed as `<provider>/<model>` wherever ambiguity is possible — in `--model`, in a quota, in a key's allowed list. `model list` prints both halves.
...
- `model app` means a model application from the Market. `model caller` means an application that *calls* Router. They are different registries.
...
- A model that is configured but does not answer is a diagnosis, not a configuration change. Route through [deciding which layer is wrong](references/olares-model-diagnosis.md) before editing anything.
