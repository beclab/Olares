...
// `olares-cli model key …` — the credential other software uses.
//
...
and "olares-cli model caller list" shows which app has one.
...
Examples:
  ...
  olares-cli model key list -o json
`,
		Args: cobra.NoArgs,
...
func renderKeyList(ctx context.Context, pc *preparedClient, w io.Writer, keys []apiKeyView) error {
	if len(keys) == 0 {
		_, err := fmt.Fprintln(w, "no keys. `olares-cli model key issue <name>` creates one; "+
...
Examples:
  olares-cli model key issue "wise indexer"
  olares-cli model key issue ci --ttl 30d --model "Qwen3.6-27B (llama.cpp)/qwen3"
  olares-cli model key issue bot --expires-at 2027-01-01T00:00:00Z -o json
`,
...
Examples:
  olares-cli model key update sk-abc123 --disable
  olares-cli model key update "wise indexer" --ttl 90d
  olares-cli model key update ci --model "openai/gpt-4o-mini" --model "openai/gpt-4o"
`,
		Args: cobra.ExactArgs(1),
		RunE: func(c *cobra.Command, args []string) error {
			if enable && disable {
...
"olares-cli model usage" still attributes past calls to it — and the key can be
...
Examples:
  olares-cli model key revoke sk-abc123 --yes
  olares-cli model key revoke "wise indexer"
`,
		Args: cobra.ExactArgs(1),
		RunE: func(c *cobra.Command, args []string) error {
...
func runKeyRevoke(ctx context.Context, f *cmdutil.Factory, ref string, assumeYes bool, outputRaw string) error {
	...
	_, err = fmt.Fprintf(os.Stdout, "%s (%s) is revoked; its usage history is kept, and "+
		"`olares-cli model key update %s --enable` brings it back\n",
...
func resolveKey(ctx context.Context, pc *preparedClient, ref string) (*apiKeyView, error) {
	...
	case 0:
		if len(keys) == 0 {
			return nil, fmt.Errorf("no key %q, and you have no keys; `olares-cli model key issue <name>` creates one", ref)
		}
		return nil, fmt.Errorf("no key %q; `olares-cli model key list` shows the ones you can see", ref)
	default:
...
func verifyAllowedModels(ctx context.Context, pc *preparedClient, refs []string) ([]string, error) {
	...
	for _, raw := range refs {
		...
		if !ok {
			return nil, fmt.Errorf("no model %q is configured; `olares-cli model list` shows what is, "+
