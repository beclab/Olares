...
func (a *upgradeGPUDriverIfNeeded) Execute(runtime connector.Runtime) error {
	...
	if needUpgrade {
		...
		err = gpu.UpdateNodeGpuLabel(context.Background(), client.Kubernetes(), &targetDriverVersionStr, ptr.To(common.CurrentVerifiedCudaVersion), ptr.To("true"), ptr.To(gpu.NvidiaCardType), ptr.To(""))
		if err != nil {
			return err
		}
		changed = true
