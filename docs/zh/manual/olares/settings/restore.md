...
3. 点击**查询快照**，获取可用快照列表。
4. 在目标快照后点击**恢复**以加载快照。
5. 如果是恢复**文件**，请指定恢复位置和目标文件夹，然后点击**开始恢复**。  
   如果是恢复 **Wise 应用**，无需指定路径，直接点击**开始恢复**即可。
</template>
<template #从-Olares-Space-链接恢复>

1. 使用 LarePass 应用扫码登录 [Olares Space](https://space.olares.com)。
2. 在**备份**页面的备份列表里找到目标备份，点击右侧**查看详情**。 
3. 点击页面右上角**恢复**按钮以获取最新快照链接，或在列表中选择指定时间快照并点击右侧**恢复**按钮获取对应链接。 
4. 将获取的链接粘贴至 Olares 恢复页面的**备份 URL** 输入框。 
5. 输入备份密码。  
6. 如果是恢复**文件**，请指定恢复位置和目标文件夹，然后点击**开始恢复**。  
   如果是恢复 **Wise 应用**，无需指定路径，直接点击**开始恢复**即可。
</template>
...
3. 将获取的链接粘贴至 Olares 恢复页面的**备份 URL** 输入框。  
4. 输入备份密码。
5. 点击**查询快照**，获取可用快照列表。
