...
// `olares-cli model provider sync-models <name|id>`
...
Example:
  olares-cli model provider sync-models ollama-local
`,
	...
	if res.ModelCount == 0 {
		_, err := fmt.Fprintf(os.Stdout,
			"The upstream reported no models, so nothing was removed. "+
				"Check that it is running and answering: `olares-cli model provider validate %s`\n",
			found.Name)
		return err
	}
	return nil
