// This is just an example,
// so you can safely delete all default props below

const options = {
	TERMINUS: 'Olares',
	MIDDLEWARE: 'Middleware',
	BROWSE: 'Browse',
	NAMESPACE: 'Namespace',
	STORAGES: 'Storages',
	NETWORKS: 'Networks',
	POSTGRES: 'Postgres',
	MONGO_DB: 'MongoDB',
	REDIS: 'Redis',
	RECORDS_PER_PAGE: 'Records per page',
	TARGET_PROJECT: 'Target project',
	DESTINATION_PROJECT: 'Destination project',
	CIDR: 'CIDR',
	STATES: 'States',
	OPERATIONS: 'Operations',
	DB_USER: 'DB user',
	DATABASE: 'Database',
	BUCKETS: 'Buckets',
	VHOST: 'Vhost',
	INDEX: 'Index',
	SUBJECT: 'Subject',
	PASSWORD: 'Password',
	HOST: 'Host',
	USER: 'User',
	MONGOS: 'Mongos',
	SIZE: 'Size',
	PROXY: 'Proxy',
	MAPPINGS: 'Mappings',
	TYPE_SEARCH: 'Enter your query',
	SELECT_INDEX: 'Select index first',
	NO_RESULT: 'No result',
	SEARCH_FIELD: 'Search for a field',
	POD_LIST: 'Pod list',
	TIMESTAMP: 'Timestamp',
	MAX_RECORDS: 'Maximum records to return',
	SYNTAX_GUIDE: 'Syntax guide',
	FOUND_HITS: 'Found {count} hits in {time} ms',
	DOC_NUM: 'Doc Num',
	SHARD_NUM: 'Shard Num',
	STORAGE_SIZE: 'Storage size',
	STORAGE_TYPE: 'Storage type',
	ACTIONS: 'Actions',
	CONFIRM: 'Confirm',
	STOP_POD: 'Stop',
	START_POD: 'Start',
	RESTART_POD: 'Restart',
	BASIC_INFOMATION: 'Basic infomation',
	OLARIS_TERMINAL: 'Olares',
	DELETE_POD: 'Delete Pod',
	SELECT_NODE: 'Select node',
	middleware: {
		app: 'App',
		app_nats: 'Publisher',
		user_database: 'DB user',
		user_other: 'User',
		type_changed_title: 'Middleware Deleted',
		type_changed_message:
			'The current middleware has been deleted and will redirect to the home page',
		deleted_types: 'Deleted middleware'
	}
};

export default options;
