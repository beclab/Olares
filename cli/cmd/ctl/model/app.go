...
// `olares-cli model app …` — model applications, installed through Router.
//
...
Examples:
  ...
  olares-cli model app catalog --category AI
`,
		Args: cobra.NoArgs,
...
func renderCatalog(w io.Writer, items []marketApp) error {
	...
	_, err := fmt.Fprintln(w, "\nAPP is the name `app install` takes. `olares-cli model provider list` shows which are already installed.")
	return err
}
...
Examples:
  olares-cli model app install qwen3-8b --watch
  olares-cli model app install qwen3-8b -o json
`,
...
func refuseReinstall(ctx context.Context, pc *preparedClient, appName string) error {
	...
	return fmt.Errorf("%s is already installed on this machine and is %s; "+
		"`olares-cli model app upgrade %q` moves it to a newer version, "+
		"`olares-cli model app uninstall %q` removes it, and "+
...
stops. Check with "olares-cli model usage" before removing something a
...
Examples:
  olares-cli model app uninstall "Qwen3.6-27B (llama.cpp)" --yes --watch
`,
		Args: cobra.ExactArgs(1),
		RunE: func(c *cobra.Command, args []string) error {
...
func runAppLifecycle(ctx context.Context, f *cmdutil.Factory, ref, action, gerund string, confirm, watch bool, outputRaw string) error {
	...
	if !found.isMarketSourced() {
		return fmt.Errorf("%s was entered by hand and has no application behind it, so there is nothing to %s; "+
			"`olares-cli model provider delete %s` removes the provider itself",
...
func reportStarted(ctx context.Context, pc *preparedClient, started startedTask, what string, watch bool, format Format) error {
	...
	if !watch {
		_, err := fmt.Fprintf(os.Stdout, "follow it with `olares-cli model app watch %s --task %d`\n",
...
Examples:
  ...
  olares-cli model app tasks <provider-id> -o json
`,
		Args: cobra.ExactArgs(1),
		RunE: func(c *cobra.Command, args []string) error {
...
Examples:
  olares-cli model app watch "Qwen3.6-27B (llama.cpp)"
  olares-cli model app watch <provider-id> --task 42
`,
		Args: cobra.ExactArgs(1),
		RunE: func(c *cobra.Command, args []string) error {
...
func reportOutcome(w io.Writer, t *installTask) error {
	...
	default:
		...
		_, err := fmt.Fprintf(w, "the event stream ended while this %s is still %s; "+
			"`olares-cli model app watch %s --task %d` picks it back up\n",
