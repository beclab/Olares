...
const props = defineProps({
	status: {
		...
		default: [TransferStatus.All] as TransferStatus[]
	}
});
...
const downloadList = computed(() => {
	const list = transferStore.cloud;
	if (list.length > 0) {
		return list
			.map((item) => {
			...
			.filter((item) => {
				if (props.status.includes(TransferStatus.All)) {
					return true;
				}

				return props.status.includes(item.status);
			})
