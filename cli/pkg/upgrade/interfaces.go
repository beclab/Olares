...
func stageTasks(u upgrader) map[string][]task.Interface {
	return map[string][]task.Interface{
		...
		StagePostUpgradeAdmin: u.UpdateOlaresVersion(),
		StageRebootNodes:      u.RebootNodes(),

		// PostUpgrade belongs after the reboot, not next to the version flip.
		// It means "the upgrade is over, check the cluster survived it", and
		// on a release that replaces a GPU driver the upgrade is not over
		// until the machines holding that driver have restarted. Run before
		// reboot-nodes it waits out its retries on device plugins that cannot
		// be ready yet, fails, and takes the reboot down with it — leaving the
		// driver installed and never loaded.
		StageVerifyCluster: u.PostUpgrade(),
	}
}
