---
outline: [2, 3]
description: 了解如何在 Olares 中管理应用环境变量，包括设置界面操作和控制面板高级修改。
head:
  - - meta
    - name: keywords
      content: Olares, 环境变量, 应用配置, 容器变量, 应用管理, 系统环境变量, 控制面板, ConfigMap, Secret
---
...
- **引用的系统环境变量**：应用引用的系统全局变量。只有当应用使用了这些变量时，它们才会显示在此页面上。这类变量在当前页面呈置灰状态，且无法直接编辑。若要修改此类变量，可参考[设置系统级环境变量](/zh/manual/manage-system-env)。
...
如需调试、功能配置或临时调整，你也可以在控制面板中直接查看和修改应用的环境变量。

:::info
在控制面板中修改的变量在应用升级后不会保留。如需永久配置，请设置[系统级环境变量](/zh/manual/manage-system-env)或使用应用自身的设置项。
:::
...
2. 在**部署**资源组下，点击 **jellyfin**，然后点击 <i class="material-symbols-outlined">edit_square</i>。
...
2. 在**配置字典**下，点击 `deerflow-config`。
    ![浏览到 DeerFlow 的配置字典](/images/zh/manual/olares/deerflow-configmap.png#bordered)
...
4. 在 `data` 部分下添加以下键值对：
   ...
   ```
   ![配置 Tavily](/images/zh/manual/olares/deerflow-configure-tavily.png#bordered)
5. 点击 **Confirm** 保存更改。
6. 返回**部署** > **deerflow**，然后点击**重启**。

   ![重启 DeerFlow](/images/zh/manual/olares/deerflow-restart.png#bordered)
...
2. 在**部署**资源组下，点击 **ollamav2**，然后点击 <i class="material-symbols-outlined">edit_square</i>。
   ![在控制面板中定位 Ollama 实例](/images/zh/manual/olares/locate-ollama-instance.png#bordered)

3. 在 YAML 编辑器中，找到 `containers` 部分，定位并检查 `env` 字段。这里的配置引用了 `nginx.conf`。

   ![编辑 Ollama 实例的 YAML](/images/zh/manual/olares/edit-yaml-ollama.png#bordered)

4. 点击 **Cancel** 关闭编辑器。

5. 展开**配置字典**，点击 `nginx-config`，然后点击右上角的 <i class="material-symbols-outlined">edit_square</i>。

   ![定位 Nginx 配置实例](/images/zh/manual/olares/locate-nginx-config.png#bordered)

6. 在 YAML 编辑器中，找到 `data` 部分，定位 `nginx.conf` 键，然后将 `proxy_read_timeout` 的值从 `300s` 改为 `600s`。

   ![编辑 Nginx 配置](/images/zh/manual/olares/edit-nginx-conf.png#bordered)

7. 点击 **Confirm**。

8. 返回**部署** > **ollamav2**，然后点击**重启**，使更改生效。
...
2. 选择 **ollamaserver-shared**，然后在**部署**资源组下，点击 **ollama**，再点击 <i class="material-symbols-outlined">edit_square</i>。

   ![在系统命名空间中定位 Ollama](/images/zh/manual/olares/locate-ollama-sys-namespace.png#bordered)

3. 在 YAML 编辑器中，找到 `containers` 部分，检查 `envFrom` 字段。这里的配置引用了 `ollama-env`。

   ![编辑 YAML](/images/zh/manual/olares/edit-yaml-envfrom.png#bordered)

4. 点击 **Cancel** 关闭编辑器。

5. 返回**配置字典**，点击 `ollama-env` 实例，然后点击右上角的 <i class="material-symbols-outlined">edit_square</i>。

   ![编辑 Ollama 环境变量](/images/zh/manual/olares/edit-ollama-env.png#bordered)

6. 在 YAML 编辑器中，找到 `data` 部分，然后将 `OLLAMA_MAX_LOADED_MODELS` 的值从 `3` 改为 `5`。

   ![编辑 Ollama 变量](/images/zh/manual/olares/modify-var-ollama.png#bordered)

7. 点击 **Confirm**。

8. 返回**部署** > **ollama**，然后点击**重启**，使更改生效。

等待状态图标变为绿色，表示新配置已加载。

### 修改保密字典中的变量

保密字典中的修改流程与配置字典相同。根据你的应用类型，遵循[常规应用](#modify-standard-apps)或 [C/S 架构应用](#modify-cs-apps)中的步骤。
...
- [系统环境变量](/zh/manual/manage-system-env)
