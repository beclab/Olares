...
迁移后，ComfyUI 会自动生成 `extra_model_paths.yaml` 配置文件，该文件告诉 ComfyUI 在 `公共` 库中模型的位置。

文件预配置如下：
- **`base_path`**：指向 `Common/comfyui/model`（共享模型目录）。

一般情况下，无需手动修改此文件。但如果你的模型存放在非默认文件夹下，可以在以下位置手动编辑并添加路径：
```
...
- **原因 1**：`extra_model_paths.yaml` 中未注册模型所在子目录，导致 ComfyUI 未扫描该目录。
- **原因 2**：模型已在 ComfyUI 模型目录中，但自定义节点定义的搜索路径不匹配。

如下示例，`公共` 目录下包含以下两个模型，但这两个模型均未被工作流节点识别。
   ```
...
2. 如果模型未出现在列表中，说明其子目录未在 `extra_model_paths.yaml` 中注册。
   ...
   在示例情景中，`ultralytics/bbox/face_yolov8m.pt` 被检测到，但 `detection/mediapipe_face_fp32.safetensors` 未被识别：

   ![模型检测到](../../public/images/manual/use-cases/comfyui-common-model-detected.png#bordered){width=49%}
...
2. 打开 `extra_model_paths.yaml`。顶部的 `base_path` 指向 `/mnt/olares-shared-model`，对应容器内的 `/Common/comfyui/model/`。
   ...
   在本示例中，`detection` 子目录未在 `extra_model_paths.yaml` 中注册，因此则需在文件底部添加 `detection: detection` 进行注册。其中，键（`detection`）代表 ComfyUI 模型目录下的子目录名称，也是工作流节点所需的模型搜索路径，值（`detection`）表示 `Common/comfyui/model/` 下对应的子目录名称。
...
2. 将所需路径映射添加到 `extra_model_paths.yaml`。例如，要让 bbox YOLO 模型可被该节点使用，需要添加如下路径：
