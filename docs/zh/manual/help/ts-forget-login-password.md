...
使用安装 Olares 时在该设备上配置的操作系统用户名和密码。通过 SSH 登录时，还需要该设备的本地 IP 地址。如果不知道 IP 地址或无法使用 SSH，请连接显示器和键盘，在设备上直接登录。其他终端访问方式请参考[访问 Olares 主机终端](../access-olares-terminal.md)。

### 步骤 2：访问设备终端

通过以下任一方式连接到 Olares 设备的主机终端：

- **SSH**：在与设备处于同一局域网的电脑上打开终端，运行 `ssh <用户名>@<设备IP>`。如果使用 Olares One，请运行 `ssh olares@<内网IP>`。
