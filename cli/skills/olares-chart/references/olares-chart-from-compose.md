...
- **`restart: no` / `on-failure`, `deploy.mode: global` and `kompose.cronjob.schedule` are all dropped** — kompose would emit a bare Pod, a DaemonSet and a CronJob, none of which Olares can scale for install / suspend / resume, so the service comes out as a replica-controlled workload with `restartPolicy: Always`. A `kompose.controller.type` label changes which kind that is but does not bring the semantics back; the notice list names the kind each service actually became. A one-shot or scheduled job therefore becomes a long-running workload — rethink it rather than shipping it. If the app genuinely needs periodic work, either schedule it inside the container or add a CronJob template by hand and keep it out of `workloadReplicas`.
- **A compose service name that starts with a digit** (`1web`) cannot become a valid Service name, and is reported rather than fixed: kompose derives the pod labels it selects on, and any Ingress backend, from the same name, so renaming only the Service would point both at nothing. Rename the compose service.
...
- **The workload kind is pinned to Deployment**; a service-level `kompose.controller.type` label is the only way to change it. `statefulset` still lands in `workloadReplicas`, so suspend/resume keeps controlling it; `daemonset` does not (DaemonSets are one-per-node — see manifest Workloads & replicas); any other value leaves that service with **no workload at all**, only its Service and PVCs.
