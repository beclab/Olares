...
3. 安装新版本并恢复模型：
    ...
    使用新的引擎基座应用（llama.cpp 引擎基座、Ollama 引擎基座、SGLang 引擎基座，或 vLLM 引擎基座）部署所需模型。
    ...
    - 如需重新下载，使用新的 Ollama 引擎基座应用部署所需模型。模型文件将自动保存到 `ollama` 子目录中。
    ...
    从应用商店重新安装新版本的 ComfyUI。

    系统会自动将现有模型文件移动到 `comfyui` 子目录中。
