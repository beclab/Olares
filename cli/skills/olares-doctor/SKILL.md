...
A **model** that is configured but does not answer is diagnosed one layer up first: [`olares-model`](../olares-model/SKILL.md) separates the gateway, its access control and the model application's own download/engine state from the pod-level failures here, and routes back when the cause is below the application.
