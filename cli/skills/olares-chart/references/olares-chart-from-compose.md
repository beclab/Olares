...
- **`kompose.hpa.*` labels render a HorizontalPodAutoscaler** that drives the same `spec.replicas` as `workloadReplicas`, which is how Olares installs, suspends and resumes an app — the two will fight. The conversion reports it and keeps the autoscaler's scale target pointed at the right workload (kompose names the target after the compose service and always calls it a Deployment), but **you** have to decide: delete the autoscaler, or keep its workload out of `workloadReplicas`.
