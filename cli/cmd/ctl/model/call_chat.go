...
— the same numbers "model usage" will later bill. --quiet drops that line for
...
"model provider get <provider>" reports.

Examples:
  olares-cli model call chat "summarise the CAP theorem in three lines"
  git diff | olares-cli model call chat --system "write a commit message"
  olares-cli model call chat "what is in this screenshot" --image shot.png
  olares-cli model call chat "list three colours as JSON" --no-stream --quiet
  olares-cli model call chat "hello" --model gpt-4o-mini -o json
`,
