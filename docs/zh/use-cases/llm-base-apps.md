...
- 如果你想跳过手动配置、快速体验模型，可以直接从 Market 安装预构建的模型应用。Market 目前提供以下预构建模型应用。这些应用打包了 Olares 验证推荐的模型与引擎组合。

  <!-- 该列表仍在调整中，最终模型应用阵容确认后将更新。 -->
...
5. 运行一个任务来测试连接。本示例使用 Olares skills 将一个应用上传并部署到 Olares。
...
- **推荐模型**：Hugging Face 上的 [`openai/gpt-oss-20b`](https://huggingface.co/openai/gpt-oss-20b)
