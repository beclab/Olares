...
// `olares-cli model user list` — GET /console/api/users
//
...
Use "olares-cli model whoami" for your own record, which any user can read.
...
these names, and "model usage --user" narrows spend to one of them.

Examples:
  olares-cli model user list
  olares-cli model user list -o json
`,
		Args: cobra.NoArgs,
