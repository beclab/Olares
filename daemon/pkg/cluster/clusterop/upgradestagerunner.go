...
type LocalUpgradeStageRunner struct {
	...
	// restartPending answers whether the stage that just ran left this machine
	// on its way down, and bootID which boot it ran on. Injected so a test
	// needs neither a real marker file nor a real reboot.
...
// rebootMarkerPresent reports whether this machine is about to reboot as part
// of the upgrade.
//
// The marker is written by the task that decides a reboot is needed, before it
// issues one, and it is what olares-cli's own RebootIfNeeded reads in a later
// stage and what olaresd reads to report the system as rebooting rather than
// briefly reporting the upgrade complete. Reading it here adds no new
// agreement between the two sides — it forwards the one they already keep.
//
// It lives on tmpfs, so a machine that has already rebooted and come back
// answers no, which is the right answer to "is a restart still coming".
func rebootMarkerPresent() bool {
	_, err := os.Stat(clistate.UpgradeRebootMarkFile)
	return err == nil
}
...
func NewLocalUpgradeStageRunner(operationsDir string, exec UpgradeStageExec) (*LocalUpgradeStageRunner, error) {
	...
	r := &LocalUpgradeStageRunner{
		store: store, exec: exec, base: context.Background(), now: time.Now,
		restartPending: rebootMarkerPresent,
		bootID:         HostBootID,
	}
...
func (r *LocalUpgradeStageRunner) run(req UpgradeStageRequest, state UpgradeStageState) {
	...
	if err != nil {
		...
		state.RestartPending = r.restartPending()
		if state.RestartPending {
			// Which boot the stage ran on, so the control node can tell a
			// machine that has since restarted from one still waiting to.
			// Only the node can say it, and only while it is still the
			// machine that ran the work. Recorded only alongside the flag it
			// qualifies: on a stage that took nothing down there is no
			// restart to prove, and a boot id would only invite one to be
			// waited for.
			if id, err := r.bootID(); err != nil {
				klog.Warningf("clusterop: read this machine's boot id after stage %s of %s: %v",
					req.Stage, req.OperationID, err)
			} else {
				state.BootID = id
			}
		}
		if state.RestartPending {
			klog.Infof("clusterop: upgrade stage %s of %s succeeded and is taking this machine down",
				req.Stage, req.OperationID)
		} else {
