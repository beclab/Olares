...
func addServiceToContainer(c *restful.Container, handler *Handler) error {
	registerContainerFilters(c, handler)
	if err := addWebhookRoutesToContainer(c, handler); err != nil {
		return err
	}
	return addAPIRoutesToContainer(c, handler)
}

func addAPIRoutesToContainer(c *restful.Container, handler *Handler) error {
	ws := newWebService()
