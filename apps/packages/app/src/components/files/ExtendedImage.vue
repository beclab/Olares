...
export default {
	...
	methods: {
		...
		updateImgSize() {
			let imgRes = this.$refs.imgex;

			const widthRatio = this.imgWidth / this.screenWidth;
			const heightRatio = this.imgHeight / this.screenHeight;
			if (widthRatio >= heightRatio) {
				imgRes.classList.add('width-fit');
				imgRes.classList.remove('height-fit');
			} else {
				imgRes.classList.add('height-fit');
				imgRes.classList.remove('width-fit');
			}
		},
...
.image-ex-img.width-fit {
	width: 100%;
	height: auto;
}

.image-ex-img.height-fit {
	height: 100%;
	width: auto;
}
</style>
