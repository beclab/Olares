...
- 电脑上已有一个本地 Obsidian 笔记库，并计划将这台电脑作为主设备。
- 已在电脑和手机上安装 LarePass。
...
1. 打开 Olares Settings，然后进入 **Applications**、**Obsidian LiveSync**、**Entrances**。

2. 将 **Authentication level** 设置为 **Public**，将 **Authentication method** 设置为 **None**，然后点击 **Submit**。
...
:::warning 直接数据库访问
将 **Authentication method** 设置为 **None** 后，Obsidian 插件可以在不弹出 Olares 登录提示的情况下连接。请使用强数据库密码，为客户端访问保持 LarePass 开启，并避免公开分享 endpoint。
:::
...
1. 在电脑上启用 LarePass VPN，以连接到 Olares。
2. 在电脑上打开 Obsidian，然后设置要同步的笔记库。
...
2. 在手机上打开 Obsidian，并为同步笔记创建一个新的本地笔记库。
3. 在 **Settings**、**Community plugins** 中安装并启用 Self-hosted LiveSync。操作流程与主设备相同。
