...
Everything here is admin-only, reads included: even listing providers refuses a non-admin, for whom `model list` is the view of what exists. Only the bare list of vendor kinds is open.

## 1. Choose the upstream kind

`model provider types` lists what Router can speak to, and for each: the provider type to pass to `--type`, how models arrive, which endpoint families it serves, how many predefined models Router already knows, and the credential fields it requires. `model provider types <vendor>` adds every optional field with its default and where to get a key, and `--models` lists the models it can import.
...
```
olares-cli model provider create --type openai --name openai-main \
...
```
olares-cli model provider models import openai-main gpt-4o gpt-4o-mini
olares-cli model provider sync-models ollama-box
olares-cli model provider models add my-endpoint llama-3.1-70b \
...
- `--supports` takes flags from `model capabilities`. Declaring one the model does not have makes a caller send a request the upstream will reject; omitting one it does have makes callers avoid a feature that works.

`model provider get <provider>` then shows the provider with every model it serves, their modes, sizes and capabilities. `model list` shows every model across every provider.
...
A provider whose source is `olares` was created by installing a model application: see [local LLM applications](olares-model-local-llm.md). `provider delete` refuses it, `provider update` cannot move its address, and its credentials are the platform's. `provider register` is the repair path when such an application exists but its Router row does not.
