...
:::warning
本文档由 AI 自动翻译，仅供参考。涉及关键操作或信息时，请以[英文原文](../../use-cases/firecrawl-1.12.6.md)为准。
:::

# 使用 Firecrawl 作为网页加载器 — Olares 1.12.6
...
要在 Open WebUI 中使用 Firecrawl，首先[将 Open WebUI 连接到模型](openwebui-1.12.6.md#配置模型后端)并配置网络搜索。[Open WebUI 网络搜索指南](openwebui-search-1.12.6.md#启用网络搜索-1)使用 SearXNG 作为示例。然后按以下步骤手动配置 Firecrawl 作为网页加载器。
...
- [在 Open WebUI 中启用网络搜索](openwebui-search-1.12.6.md)：在 Open WebUI 中配置 SearXNG 和网络搜索。
