...
func TestGenSharedEntranceURLForUser(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			viewer: "brucedai", domain: "olares.com",
			want: "https://" + SharedEntranceHostPrefix("a5be2268", "ollamav2") + ".brucedai.olares.com",
		},
		{
...
func TestGenSharedEntranceURLForUser_DiffersByViewer(t *testing.T) {
	a := GenSharedEntranceURLForUser("a5be2268", "ollamav2", "brucedai", "olares.com")
	b := GenSharedEntranceURLForUser("a5be2268", "ollamav2", "alice", "olares.com")
	if a == b {
		t.Fatalf("viewer change must change URL (got %q == %q)", a, b)
	}
	if !strings.Contains(a, ".brucedai.") || !strings.Contains(b, ".alice.") {
...
func TestURL_LogicalPattern_Consistency(t *testing.T) {
	...
	url := GenSharedEntranceURLForUser("a5be2268", "ollamav2", "brucedai", "olares.com")
	wantHost := strings.Replace(pat, "*", "brucedai", 1)
