...
A **model** application is a special case: [`olares-model`](../olares-model/SKILL.md) installs it and creates the gateway provider that routes to it in one step, and it owns the model's own download and engine state afterwards. Come back here to stop, resume, cancel, or install an engine base that needs its model chosen on the install form.
