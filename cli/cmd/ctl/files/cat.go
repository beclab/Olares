...
//
// Streams the raw bytes of a single remote file to stdout. ONE wire
// flow, uniform across every supported namespace (drive / sync /
// cache / external / awss3 / google / dropbox / tencent / share):
//
//	GET /api/raw/<fileType>/<extend><encSubPath>?inline=true
//
// Same endpoint `files download` uses; `inline=true` only affects
// Content-Disposition (the body is identical). The earlier
// per-cloud dispatch through `/drive/download_sync_stream` was
// removed — that proxy is no longer the supported wire path
// (the unified /api/raw/ route now resolves cloud-bridge fetches
// server-side, returning raw bytes for awss3/google/dropbox/tencent
// just like for the files-backend-managed namespaces). See
// internal/files/download.Client.StreamRaw for the wire details.
//
...
Wire endpoint: GET /api/raw/<fileType>/<extend><encSubPath>?inline=true,
uniform across drive / sync / cache / external / awss3 / google /
dropbox / tencent. (Earlier CLI builds routed cloud drives through
` + "`/drive/download_sync_stream`" + ` instead — that proxy has been
retired and the unified /api/raw/ route now serves cloud-bridge
fetches server-side.)
...
func runCat(ctx context.Context, f *cmdutil.Factory, out io.Writer, remoteArg string) error {
	...
	// Uniform wire path: GET /api/raw/<fileType>/<extend><encSubPath>?inline=true.
	// Resolves correctly for every supported namespace including
	// cloud drives (awss3 / google / dropbox / tencent) — the server
	// dispatches the cloud-bridge fetch internally now, so the CLI
	// no longer needs the per-namespace branch the earlier
	// /drive/download_sync_stream path required.
