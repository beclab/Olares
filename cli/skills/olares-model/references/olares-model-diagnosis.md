...
| Layer | Check |
|---|---|
| The CLI's reach into Router | `model status` |
| Router's own configuration | `model provider get`, `model list`, `model default show` |
| Access control | `model key list`, `model quota list`, `model usage list --status failed` |
| The model application | `model local status`, `model local progress` |
| Below the application | [`olares-doctor`](../../olares-doctor/SKILL.md) — pods, events, images, resources |

## Start at the top

`model status` answers the first layer completely: whether Router is installed, which listing and entrance it was reached at, whether it is healthy, and what role it gives you. Three failures look alike and are not:

- **"Router is not installed"** on an admin profile usually means it really is not, and `olares-cli model app catalog` will fail too. Install it from the Market.
...
```
olares-cli model list
olares-cli model provider get <provider>
olares-cli model default show
```

- **A provider missing from `provider list`** is usually not missing. Router hides an `olares`-sourced provider while its application is not running; `model provider get <id>` shows it anyway, and `olares-cli market status <app>` says why.
...
- **A model that exists but is not offered** is disabled: `model list --disabled` finds it, `provider models update ... --enable` restores it.
...
A configuration that is right and a caller that is refused look identical from the outside. `model usage list --status failed --limit 20` separates them: a refused call that Router recorded has its error code in the row, and a call that produced no row at all never got past authentication.

- `invalid_api_key` with type `authentication_error` — Router refusing this key. Check `model key list` for revoked, expired, or an allowlist that excludes the model.
- An authentication error **without** that type — the vendor refusing Router's credential. `model provider validate <provider>` confirms it, and `provider history` shows what was last rotated.
- `quota_exceeded` — `model quota list` names which of the three ceilings bit.
...
```
olares-cli model local status <app>
olares-cli model local progress <app>
```
...
- **An install that never finished** — `model app tasks <provider>` carries the Market's own reason.
