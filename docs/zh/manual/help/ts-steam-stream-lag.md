...
在 Olares 上串流 Steam 游戏时，如果感觉卡顿或有延迟，可参考本指南进行排查。
...
- **nginx 重启**：nginx 重启会打断串流。在 Olares 1.12.4 版本中，nginx 会定期重启。1.12.5 版本已修复定期重启问题，但安装或卸载应用，以及其他系统事件，在某些情况下仍可能触发 nginx 重启。
- **网络连接**：Olares 设备使用的是 Wi-Fi 而不是有线网络，导致带宽较低或延迟较高。
- **GPU 分配**：GPU 未设置为**应用独占**模式，导致 Steam Headless 需要与其他应用竞争 GPU 资源。
- **工作模式**：系统运行在**静音模式**下，可能会降低游戏时的性能。
