...
`olares-cli` 的用户模式和集群内模式主要是给 AI 智能体用的，而不是让人逐条敲命令。为此，`olares-cli` 附带了一组 Agent Skills，每组命令对应一个。每个 skill 会告诉智能体各命令的作用、哪些参数重要、如何鉴权，以及遇到常见错误怎么处理。

## 了解 Agent Skills

每个 Agent Skill 就是一份 `SKILL.md`，AI 运行时会把它当作工具定义加载进来。当智能体收到“列出我 Olares Home 文件夹里的文件”这类请求时，它会对照已加载的 skill，找到对应的命令（`olares-cli files ls /drive/Home`），然后代你运行。

这些 skill 都放在 Olares 仓库的 [`cli/skills/`](https://github.com/beclab/Olares/tree/main/cli/skills) 目录下，每个 skill 由一份 `SKILL.md` 和一个 `references/` 文件夹组成。`SKILL.md` 负责把请求引导到正确的命令，并讲清楚通用概念和常见错误怎么处理。`references/` 则给每个较复杂的子命令单独准备一个文件，用来补充那些 `--help` 里查不到、又不方便塞进 `SKILL.md` 的细节。
...
如果你是用 `npx @olares/cli@latest install` 安装的 CLI，这些 skills 已经一并装好，可以跳过本节。如果是单独安装的 CLI，运行下面的命令把六个 skill 一次装好：

```bash
npx skills add beclab/Olares -y -g
```

这会把 skills 装进你正在用的智能体，比如 Cursor 或 Claude Code。之后只要你提到 Olares 相关的任务，智能体就会自动加载对应的 skill。由于这条命令会把 `olares-shared` 一起装上，“先装 shared”的要求自然就满足了。
...
加载这些包后，就能用自然语言操作 Olares，由智能体决定运行哪条 CLI 命令。例如：

```plain
# 通过 olares-files skill 列出文件
列出我 Olares 设备上 Home 文件夹里的文件

# 通过 olares-market skill 安装应用
从 Market 安装 Firefox，装好后告诉我

# 通过 olares-dashboard skill 查看资源占用
告诉我哪些应用占用了超过 1 GB 内存
```

:::tip
如果智能体没有自动加载 Olares skills，用斜杠命令显式调用它们。
:::
