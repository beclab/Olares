...
// The kube files are regenerated by two different sets of tasks with two
// different scopes, and the split is not cosmetic.
//
// On k3s each machine has its own k3s.service carrying its own kubelet flags,
// so regenerating is something every node does to itself. On kubeadm the same
// job is done by re-running kubeadm init against the control plane's config,
// which writes control-plane manifests and certificates: on a worker that is
// not a regeneration, it is a promotion. Running one function on every node
// would do the right thing for one distribution and rebuild the cluster for
// the other.
//
// Both end in the same wait. Whatever restarts the apiserver has to see it
// answer again before anything downstream assumes it can talk to it, and
// keeping the wait inside these functions is what makes them safe to place in
// whichever stage suits their scope.
func apiServerBackUp() task.Interface {
...
// regenerateKubeFilesOnNode is the half every machine does to itself. It is
// empty on kubeadm, where nothing about this is a node's own business.
func regenerateKubeFilesOnNode() []task.Interface {
	if phase.GetKubeType() != common.K3s {
		return nil
	}
	return []task.Interface{
		&task.LocalTask{
...
// regenerateKubeFilesOnControlNode is the kubeadm half, and only the control
// node may run it. Empty on k3s.
//
// A kubeadm cluster's compute nodes therefore keep the reserve they were
// joined with, which is what they did before any of this was split up. Giving
// them the new one needs a way to rewrite a worker's kubelet config that does
// not go through kubeadm init, and inventing one here — untested, on the
// distribution neither the test rig nor the default install uses — would risk
// far more than the reserve is worth.
func regenerateKubeFilesOnControlNode() []task.Interface {
	if phase.GetKubeType() == common.K3s {
		return nil
	}
	return []task.Interface{
...
func (a *upgradeGPUDriverIfNeeded) Execute(runtime connector.Runtime) error {
	...
	if needReboot {
		...
		// the next task. It lives under /run and is cleared automatically
		// once the machine reboots.
		//
		// Failing to write it fails this task, because on a cluster the
		// marker is the only thing that remembers a reboot is owed. The two
		// tasks no longer share a process — the driver is installed in one
		// stage and RebootIfNeeded runs in a later one — so the pipeline
		// cache that used to carry the answer is empty by then, and the
		// orchestrator reads the same marker to decide whether to wait for
		// this machine to go down. Without it the node keeps the new driver,
		// never restarts into it, and nothing anywhere says so. That is worth
		// stopping for; it was only ever a warning when the marker did no
		// more than keep olaresd reporting "rebooting" for a few seconds.
		if _, err := runtime.GetRunner().SudoCmd(fmt.Sprintf("touch %s", clistate.UpgradeRebootMarkFile), false, false); err != nil {
			return errors.Wrapf(errors.WithStack(err),
				"failed to record that this machine owes a reboot (%s)", clistate.UpgradeRebootMarkFile)
		}
	}
	return nil
}
