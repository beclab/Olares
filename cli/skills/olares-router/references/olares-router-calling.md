...
| `model_at_capacity` with a 503 | The engine is already serving every request it was launched for, so this one was refused instead of queued behind them. Nobody can raise that number without restarting the application wider — wait for the `Retry-After` the message names, and do not answer it with more concurrency |
