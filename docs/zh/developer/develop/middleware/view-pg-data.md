...
在建立连接之前，需要从控制面板获取 PostgreSQL 的连接信息。

1. 从启动台打开控制面板。
2. 在左侧导航栏中找到中间件，并选择 **Postgres**。
3. 记录信息面板中的以下信息：
    - **主机**：用于在 Bytebase 中建立连接。
    - **用户**：用于在 Bytebase 中建立连接。
    - **密码**：用于 CLI 和 Bytebase。

    ![连接信息](/images/zh/manual/developer/mw-pg-details.png){width=60% style="margin-left:0"}
