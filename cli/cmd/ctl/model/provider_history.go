...
// `olares-cli model provider history <name|id>`
// `olares-cli model provider rollback <name|id> <version>`
...
Example:
  olares-cli model provider history openai-prod
`,
...
Example:
  olares-cli model provider rollback openai-prod 2 --note "new key was wrong"
`,
...
func runProviderRollback(ctx context.Context, f *cmdutil.Factory, ref, versionRaw, note, outputRaw string) error {
	...
	if err != nil || version < 1 {
		return fmt.Errorf("version must be a positive number; `olares-cli model provider history %s` lists them", ref)
	}
	pc, err := prepare(ctx, f)
	if err != nil {
		return err
	...
	_, err = fmt.Fprintf(os.Stdout,
		"\nRestoring does not re-check the upstream: `olares-cli model provider validate %s`\n",
		res.Provider.Name)
	return err
}
