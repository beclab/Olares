...
// `olares-cli router provider update <name|id>`
...
Examples:
  olares-cli router provider update openai-prod --status disabled
  printf '{"openai_api_key":"%s"}' "$NEW_KEY" |
    olares-cli router provider update openai-prod --credentials-json - \
	...
	if req.Credentials != nil {
		_, err := fmt.Fprintf(os.Stdout,
			"\ncredentials now at version %d. Confirm the upstream accepts them: "+
				"`olares-cli router provider validate %s`\n",
