...
3. 如果该应用支持多种硬件加速器，在**资源**区域配置部署信息：
    ...
    <!--![Accelerator resources](/images/zh/manual/olares/market-accelerator.png#bordered)-->
...
在安装应用之前，系统会评估你当前的硬件和集群容量。如果你的环境不满足所选配置，**资源**面板内会直接显示错误信息：

- “无法安装。Olares 集群未配置所需的加速器。”

    该应用需要特定的计算硬件，但你的设备或集群上未安装或未检测到相应硬件。出现这种情况时，资源下拉列表会显示 **N/A**，不支持的硬件选项将呈灰色且无法选择。

- “Olares 集群上{资源}不足，无法安装到指定类型的加速器上。”

    你的系统拥有正确的硬件类型（例如 NVIDIA GPU），但其当前可用容量（如 GPU 显存）不足以运行该应用。你必须停止其他正在运行的应用，以释放该加速器所需的资源。

- “无法安装。你的{资源}可用配额不足。”

    你的个人账户已用完分配的资源限额（例如 CPU 或 Memory）。尝试停止不使用的应用以释放分配的资源，或联系管理员申请增加配额。
