...
--no-wait returns the task id instead of waiting. "model call ocr --task <id>"
...
Examples:
  olares-cli model call ocr invoice.png
  olares-cli model call ocr book.pdf --pages 1-10
  olares-cli model call ocr scan.pdf --no-wait
  olares-cli model call ocr --task 9f2c1b40
  olares-cli model call ocr --task 9f2c1b40 --cancel
`,
...
func runCallOCR(ctx context.Context, f *cmdutil.Factory, opts ocrOptions) error {
	...
	default:
		...
		if !opts.Wait {
			...
			fmt.Printf("`olares-cli model call ocr --task %s` reads it when it is done\n", task.ID)
			return nil
		}
	}
	...
	for {
		...
		if time.Now().After(deadline) {
			return fmt.Errorf("task %s is still %s after %s; it keeps running — "+
				"`olares-cli model call ocr --task %s` picks it up",
