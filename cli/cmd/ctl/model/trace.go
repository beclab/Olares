...
// `olares-cli model trace …` — the spans behind a call.
//
...
Examples:
  ...
  olares-cli model trace list --limit 10 -o json
`,
		Args: cobra.NoArgs,
...
Examples:
  olares-cli model trace get 4bf92f3577b34da6a3ce929d0e0e4736
  olares-cli model trace get 4bf92f... --content
  olares-cli model trace get 4bf92f... -o json
`,
...
func runTraceGet(ctx context.Context, f *cmdutil.Factory, traceID string, withContent bool, outputRaw string) error {
	...
	if traceID == "" {
		return fmt.Errorf("a trace id is required; `olares-cli model trace list` has them in the TRACE column")
	}
	pc, err := prepare(ctx, f)
	if err != nil {
		return err
...
Examples:
  ...
  olares-cli model trace capture --on
  olares-cli model trace capture --off
`,
		Args: cobra.NoArgs,
		RunE: func(c *cobra.Command, _ []string) error {
			if on && off {
...
func traceRouteErr(err error) error {
	...
	if errors.As(err, &re) && re.Status == 404 && re.Code == "" {
		return fmt.Errorf("this Router has observability switched off, so it stores no traces. " +
			"Turn it on in the deployment's configuration; `olares-cli model usage` works either way, " +
