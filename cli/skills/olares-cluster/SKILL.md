...
| [`cli/cmd/ctl/cluster/pod/`](cli/cmd/ctl/cluster/pod) | `cluster pod` verbs (`list`, `get` with `--watch`, `yaml`, `events`, `logs`, `delete`, `restart`); exports `pod.Get` + `pod.RunList` + `pod.RunLogs` + `pod.LogsOptions` + `pod.RunDelete` + `pod.Event` for sibling packages. |
...
| [`cli/cmd/ctl/cluster/job/`](cli/cmd/ctl/cluster/job) | `cluster job` verbs (`list`, `get`, `yaml`, `pods`, `events`, `rerun`). Exports `job.Get` for siblings. `pods` delegates to `pod.RunList` with `controller-uid=<uid>`; `events` reuses `pod.Event`; `rerun` POSTs to the KubeSphere operations API. |
