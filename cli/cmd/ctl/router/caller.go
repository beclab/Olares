...
// `olares-cli router caller …` — the applications that call Router.
//
// GET    /console/api/apps
// DELETE /console/api/apps/:id
//
// These are consumers, not the model applications `router app` installs. An
...
completion — and not the model applications that serve models. "olares-cli
router app" is the other one.
...
Examples:
  ...
  olares-cli router caller list --all -o json
`,
		Args: cobra.NoArgs,
...
func renderCallerList(w io.Writer, apps []callerApp, archived int, includeGone bool) error {
	...
	_, err := fmt.Fprintln(w, "\nHAS KEY is the internal key Router issued to the application itself. "+
		"It is not readable and is not one of `olares-cli router key list`.")
	return err
}
...
Examples:
  olares-cli router caller archive wise --yes
`,
