...
如果你通过脚本安装完成[Olares 安装和初始配置](../get-started/install-olares.md)后，可使用以下步骤激活 Olares 实例：
...
<!--@include: ../get-started/install-and-activate-olares.md{10,19}-->
...
登录 Olares 时，系统会提示你完成双因素验证。你可以在 LarePass 手机端直接确认登录，或手动输入 6 位一次性验证码。
...
- **手动输入验证码**：
    ...
    2. 通过以下任一方式获取 6 位验证码：
        - **手机端**：打开 LarePass 应用后进入**设置**。在**我的 Olares** 卡片里，点击身份验证器，查看一次性验证码。
        ![OTP](/images/manual/larepass/otp-larepass1.png#bordered){width=95%}

        - **电脑端**：打开 LarePass 桌面版后进入 **Vault**。使用列表第一条显示的一次性验证码。
        ![OTP desktop](/images/manual/larepass/otp-larepass-desktop1.png#bordered){width=95%}

    3. 返回安装向导页面，输入验证码完成登录。  

:::tip 提示
验证码具有时效性，请在失效前输入。如果验证码失效，系统会自动生成新验证码。
:::
