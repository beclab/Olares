...
* **普通用户**：可以在应用市场中一键部署 ComfyUI、Vane（原 Perplexica）等复杂的应用程序。
...
LarePass 是 Olares 的官方客户端，充当连接设备的安全桥梁。它内置专用网络功能，可实现无缝远程访问和跨设备文件同步。目前，设备激活流程必须通过 LarePass 完成。
...
* **远程访问 (FRP)**：内置 FRP 服务提供每月 2 GB 免费流量，超出部分可选付费。免费方案是通过 LarePass 专用网络访问，或自行搭建 FRP 服务器。
