...
olares-cli model default set --mode chat=<provider>/<model>
olares-cli model default set --mine --mode chat=<provider>/<model>
olares-cli model default clear --mode chat
```
...
```
olares-cli model key issue "ci-runner" --ttl 30d --model openai-main/gpt-4o
olares-cli model key list
olares-cli model key update ci-runner --disable
olares-cli model key revoke ci-runner
```
...
A non-admin sees and manages their own keys. The key `model call` keeps for this machine is an ordinary key in this list — see [calling a model](olares-model-calling.md).
...
```
olares-cli model quota set --key ci-runner --budget 50 --warn-at 90
olares-cli model quota set --user alice --rpm 60
olares-cli model quota set --model openai-main/gpt-4o --tpm 100000
olares-cli model quota list
olares-cli model quota clear --key ci-runner --budget
```
...
Router's roles are its own: being an Olares admin is what makes you a Router admin, but the user list, the roles and the disabled state live in Router. `model whoami` reports what it decided about you.

## The applications that call Router

```
olares-cli model caller list
olares-cli model caller archive <app>
```
...
Do not confuse this with `model app`, which is a model application from the Market. A caller is anything that consumes models; a model application is something that serves them. One machine can have both, and the same application can appear in both lists for good reason.
