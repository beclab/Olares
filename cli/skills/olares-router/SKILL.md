...
| generating a picture, a clip, a track or a mesh | `call image/video/music/3d` — the four whose work outlives the request; `music` and `3d` require `--model`, and `call music format/draft/cancel` are music's own | [calling a model](references/olares-router-calling.md) |
