...
export default class DropboxDataAPI extends DriveDataAPI {
	...
	formatPathtoUrl(path: string): string {
		return `${path}?src=${DriveType.Dropbox}`;
	}
	...
	async copy(el: FileItem, type: string): Promise<CopyStoragesType> {
		const from = this.breadcrumbsBase + el.path.slice(14);
	...
	async paste(
		...
		for (let i = 0; i < operateinStore.copyFiles.length; i++) {
			...
			// const to = path + encodeUrl(element.name);
	...
	async deleteItem(items: FileItem[]): Promise<void> {
		...
		for (let i = 0; i < items.length; i++) {
			const item = items[i];
			promises.push(remove(item.path + '?src=' + DriveType.Dropbox));
		}
	...
	async createDir(dirName: string, path: string): Promise<void> {
		let url =
			path + '/' + encodeURIComponent(dirName) + '?src=' + DriveType.Dropbox;
		url = url.replace('//', '/');
	...
	getPreviewURL(file: any, thumb: string): string {
		...
		return createURL(
			'api/preview/' + thumb + decodeURIComponent(pathSplit),
			params
		);
	}
	...
	getDownloadURL(file: any, inline: boolean): string {
		...
		const url = createURL('api/raw' + decodeURIComponent(file.path), params);
		return url;
	}
		...
		if (['text', 'txt', 'textImmutable'].includes(file.type)) {
			try {
				const res = await this.commonAxios.get(
					`/api/raw${file.path}?src=${DriveType.Dropbox}`,
					{}
				);
				file.content = res;
			} catch (error) {
