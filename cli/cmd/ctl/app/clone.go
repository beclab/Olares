...
Examples:
  olares-cli app clone firefox --title "Firefox Cloned"
  olares-cli app clone myapp --title "MyApp Cloned" --env API_URL=http://dev.example.com
  olares-cli app clone myapp --title "MyApp Cloned" --entrance-title ui="New UI" --entrance-title api="New API"`,
...
func runClone(opts *AppOptions, appName string) error {
	mc, err := opts.prepare()
	if err != nil {
	...
	opts.info("Cloning '%s' as '%s' from '%s' for user '%s'...", appName, title, source, mc.user)
