...
func TestRequireComputeBackendVersion(t *testing.T) {
	...
	err := requireComputeBackendVersion(context.Background(), cmdutil.NewFactory())
	if err == nil {
		t.Fatal("expected version gate error")
	}
	for _, want := range []string{"settings compute", "requires Olares >= 1.12.6", "settings gpu list"} {
	...
	if err := requireComputeBackendVersion(context.Background(), cmdutil.NewFactory()); err != nil {
