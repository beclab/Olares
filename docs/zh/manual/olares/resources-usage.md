...
head:
  - - meta
    - name: keywords
      content: Olares, 系统监控, 资源使用, CPU 使用率, 内存使用率, 磁盘使用率, GPU 监控, 仪表盘
---
...
内存、显存和磁盘空间是三种不同的资源限制。可根据现象选择下一步：

| 资源 | 影响范围 | 空间不足或占满时 |
|---|---|---|
| 内存（RAM） | 正在运行的应用和系统进程 | 应用无法安装或恢复，或者停止应用后内存仍未释放时，请参考[停止应用后内存不足或没有释放](../help/ts-free-memory.md)。 |
| 显存（VRAM） | 分配给 GPU 等加速器的模型和应用 | GPU 应用一直处于停止状态，或无法获得所需显存时，请参考 [GPU 应用安装或恢复后处于暂停状态](../help/ts-vram-shortage.md)。 |
| 磁盘空间 | 持久文件、应用数据、已下载模型和容器镜像 | 先通过**占用分析**找到空间不足的文件系统。如需删除不用的模型或检查后确认无用的镜像，请参考[清理磁盘空间](../free-up-disk-space.md)。 |

停止应用可以释放内存或显存，但不会自动删除磁盘中的模型、应用数据或容器镜像。同样，删除无用镜像只能释放磁盘空间，不会增加可用内存或显存。
