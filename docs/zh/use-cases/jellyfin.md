...
启用 Jellyfin 的硬件加速后，服务器在需要转码时会调用 CPU 内置 GPU 的多媒体引擎，处理视频解码、编码、缩放及部分色调映射等任务，从而减轻纯软件解码对 CPU 的压力。这对于播放 4K 高码率片源、需降分辨率或降码率、客户端不支持原始编码格式、HDR 转 SDR，或需内嵌字幕等场景尤为有用。需要注意，Jellyfin 默认仍会优先采用 Direct Play 或 Remux 直放策略，只有在设备不兼容或网络带宽不足必须转码时，才会用到硬件加速。

下面以在 Olares One 设备上启用 Intel QSV 硬件加速为例，介绍具体操作步骤：

1. 请先在 Olares Market 中升级 Jellyfin，确保版本版本为 1.0.21 或以上。
2. 进入 **设置** > **应用** > **Jellyfin** > **管理环境变量**。
   - 将 `ENABLE_HW_ACCEL` 变量设置为 `true`；
   - 填写主机的 `VIDEO_GID` 和 `RENDER_GID`，Olares One 设备默认值分别为 `44` 和 `994`。如使用其他设备，可在 **控制面板** 内打开终端，并输入下列命令查询对应值：
      ``` bash
      getent group video
      getent group render
      ls -ln /dev/dri
   ...
   - 保存设置后，系统会自动重启 Jellyfin 容器。

3. 打开 Jellyfin 的 **Dashboard**（点击 ≡ 图标 > Dashboard），依次进入 **Playback** > **Transcoding**。
4. 在 **Hardware acceleration** 选项下，根据你 Olares 设备的硬件配置选择合适的加速方式。以 Olares One 的 Intel QSV 为例，推荐勾选 `H264`、`HEVC`、`HEVC 10bit`，其余选项保持默认即可。更多详细配置请参考 Jellyfin 官方的 [转码文档](https://jellyfin.org/docs/general/post-install/transcoding/)。

   ![启用转码](/images/manual/use-cases/jellyfin-transcoding.png#bordered){width=90%}

:::tip 最佳实践建议
建议家庭影院用户优先让主电视或播放器支持常见的 HEVC Main10、HDR10、SRT 字幕和主流音频格式，以便尽量实现 Direct Play 直放。硬件加速与转码的目的，是为了解决兼容性及远程或弱网络环境下的播放问题，并非用于替代直放。
另外，开启 `ENABLE_HW_ACCEL` 后，Olares 会赋予 Jellyfin 访问 DRM 的额外权限，因此请只在确需硬件加速时再启用该功能。
:::
