...
3. Package (optional for the PR — the folder is what gets committed, not the `.tgz`):
   ...
   ```
4. Verify folder name constraints (used in the PR title and GitBot validation):
   - Lowercase letters and digits only
   - **No hyphens** (`-`)
   - <= 30 characters
   - Must match `Chart.yaml` `name`, `metadata.name`, and the PR title folder segment

5. Add an **`owners` file** (no extension) in the chart root:
   ```yaml
   owners:
   - <github-username>
