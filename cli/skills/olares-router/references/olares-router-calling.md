...
`music alignment <id>` reads a finished track's lyric timeline: each line with the second it starts and ends. A player needs that to highlight the current line and to seek by tapping one, and it is derivable from neither the lyrics nor the audio — the model decides how the words fall across the sections. Router answers it from the provider and upstream id already sealed onto the generation, so it returns to the model that made the track rather than resolving one again, and nothing is regenerated. Not every track has one: the generation must have completed, and an older model application does not serve timelines at all, which comes back as `lyrics_alignment_unavailable` rather than as invented timings.

Two lyric fields come back from `format` and `draft`, and they are deliberately different. `effective_lyrics` is what the song says, for a person to read; `conditioning_lyrics` is the same words spelled for the model — phoneme hints, section markers, whatever the engine needs to pronounce them. A player shows one and the model sings the other, and while there was a single field the caller had to pick which of those to get wrong.
...
What the binding cannot repair is the engine losing the task. Only the id was ever Router's: the work and the result live in engine memory, expire 1800 seconds after the work finishes, and go with a pod or engine restart. Router answers `audio_task_lost` and does not resubmit — re-running an hour of audio is not a decision it makes for a caller. That is a different failure from a finished result ageing out, and the two read differently here. The `model` printed in a receipt is a routing reference, not the engine's canonical model id; preserve it for legacy and cross-gateway recovery.

`listen` and `diarize --stream` are the two CLI verbs that open a WebSocket rather than uploading; Router's complete audio protocol surface also includes `/v1/audio/speech/stream` for streaming TTS. The two input verbs read 16-bit mono PCM from a file or standard input and default `--sample-rate` to 16000. The flag must match the actual PCM and a sample rate advertised by the selected model's operation catalogue. They print partial results as they arrive and need models declaring `supports_stt_stream` and `supports_diar_stream` — separate applications from their batch counterparts.
