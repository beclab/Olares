...
| Template | `MODEL_SOURCE` | `MODEL_NAME` | `ENGINE_ARGS` | Required accelerator memory |
|---|---|---|---|---|
| llama.cpp | `hf://owner/repo-GGUF --include file.gguf` | `owner/repo-GGUF:quant` | for example `-c 8192 -ngl all -fa on` | `LLAMACPP_REQUIRED_GPU_MEMORY` for `nvidia`; size pod memory for `nvidia-gb10` |
| Ollama | `ollama://tag` or an HF GGUF source | client model alias | for example `OLLAMA_NUM_CTX=8192 OLLAMA_KEEP_ALIVE=30m` | `OLLAMA_REQUIRED_GPU_MEMORY` for `nvidia`; size pod memory for `nvidia-gb10` |
| vLLM | `hf://owner/repo` | `owner/repo` | for example `--max-model-len 8192 --gpu-memory-utilization 0.9 --tensor-parallel-size 1` | `VLLM_REQUIRED_GPU_MEMORY` for `nvidia`; size pod memory for `nvidia-gb10` |
| SGLang | `hf://owner/repo` | `owner/repo` | for example `--context-length 8192 --mem-fraction-static 0.8 --tp 1` | `SGLANG_REQUIRED_GPU_MEMORY` for `nvidia`; size pod memory for `nvidia-gb10` |
...
Omitting `--source` intentionally uses `market.olares`. Use `--compute-mode nvidia-gb10` for a compatible GB10 node and choose memory settings for its unified-memory model.
