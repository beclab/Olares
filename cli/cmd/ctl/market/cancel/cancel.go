// Package cancel builds the `market cancel` request for the detected Olares
...
// Build returns the cancel request (DELETE /apps/{name}/install), choosing the
// body shape by backend version.
func Build(atLeast126 bool, appName, source, version string) (method, path string, body any) {
	if atLeast126 {
		return build1_12_6(appName, source, version)
	}
	return build1_12_5(appName, source, version)
}
