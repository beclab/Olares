...
* **端点配置**：定义应用程序的网络地址和路由配置。
* **访问策略**：控制访问应用程序所需的身份验证方法。
...
**端点配置**面板允许你通过个性化 URL 定义如何从外部访问 Olares 上的应用。
...
- **设置自定义域名**：点击 <i class="material-symbols-outlined">add</i> 可使用自定义路由 ID 替换默认路由 ID。例如，将其设置为 `jellyfin` 后，应用既可以通过 `https://7e89d2a1.alexmiles.olares.com` 访问，也可以通过 `https://jellyfin.alexmiles.olares.com` 访问。

- **设置自定义域名**：点击 <i class="material-symbols-outlined">add</i> 可为该应用添加自定义域名，例如 `app.yourdomain.com`。在域名生效前，你还需要完成相应的 DNS 配置。详细说明请参阅[自定义应用 URL](custom-app-domain.md)。
