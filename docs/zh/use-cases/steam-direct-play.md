...
在大多数 Olares 设备上，连接显示器只是开始。由于 Steam Headless 默认针对远程串流做了优化，你需要手动配置显示和音频设置，才能在本机玩游戏。

:::tip Olares One 用户
Olares One 已预配置好显示和音频，无需手动设置。如果你使用的是 Olares One，只需完成下方的第 1 步安装并配置 Steam Headless，即可跳过声卡、显示和音频的手动配置（第 2–4 步），直接前往[下载并畅玩游戏](#_5-下载并畅玩游戏)。
:::
...
:::tip 安装失败请重试
如果由于网络问题导致 Steam 安装或更新失败，在 Steam Headless 控制台左上角的菜单，导航至 **Applications** > **Internet** > **Steam** 重新启动安装。
:::
...
:::info Olares One
无需执行第 2–4 步。Olares One 已预配置好显示和音频，可直接前往[下载并畅玩游戏](#_5-下载并畅玩游戏)。
:::
...
- 验证更改：确认你的缩进与现有行保持一致。
- 必要时恢复：如果 YAML 更改后应用无法启动，检查**修改记录**，删除你新增的行即可恢复之前的配置。
    ...
    # 在末尾添加以下条目：
        - name: snd
          hostPath:
            ...
            type: ''
   ```
4. 找到`volumeMounts`部分，在末尾添加以下挂载点：
   ```yaml
       volumeMounts:
...
2. 点击 **steam-headless** 旁的终端图标，进入容器 Shell。
   ![访问容器 Shell](/images/zh/manual/use-cases/steam-access-container-shell.png#bordered)
...
4. 如果输出结果包含`Mono: Playback [off]`，则表示该通道已静音。运行以下命令取消静音：
   ...
   ```
现在，音频会直接从你的显示器或扬声器播放。你可以开始玩游戏了。

## 5. 下载并畅玩游戏

1. 在 Steam 中，进入**库**查看你的游戏。
2. 选择想玩的游戏，点击**安装**。
3. 下载完成后，点击**开始游戏**。
