...
Ticket 应用内置了日志收集功能，你可以直接提交工单，无需先手动导出日志。
...
4. 在**系统日志** 中，展开该区域并点击**采集日志**，即可自动收集并附加系统日志。
   ...
   :::tip 不想使用自动收集？
   你也可以从 Olares **设置 > 高级 > 导出系统日志** 手动导出系统日志，然后在**附件**中上传。详细步骤，参阅[导出系统日志](/zh/manual/olares/settings/developer.md#导出系统日志)。
   :::
...
提交工单后，你可以跟踪工单状态、查看 Olares 团队的回复，并通过添加回复继续沟通。
...
你也可以直接通过网页浏览器在 Olares Space 中创建和管理支持工单。这种方式无需安装单独的 Ticket 应用，也无需能够访问 Olares 设备，除非你需要使用 `olares-cli` 上传日志。
