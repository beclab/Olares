...
- **`restart: no` / `on-failure`, `deploy.mode: global` and `kompose.cronjob.schedule`** are rendered as Deployments anyway (kompose would emit a bare Pod, a DaemonSet and a CronJob), because Olares installs, suspends and resumes apps by scaling replicas. A one-shot or scheduled job therefore comes out as a long-running workload — rethink it rather than shipping it. If the app genuinely needs periodic work, either schedule it inside the container or add a CronJob template by hand and keep it out of `workloadReplicas`.
