---
outline: [2,3]
description: 当 Olares 有多个 GPU 时，配置 GPU 模式、在 GPU 之间重新分配应用，并管理 GPU 访问权限。
---
# 管理多 GPU 资源

本问为你介绍 Olares 有多个 GPU 时，如何管理 GPU 模式和应用分配。每个 GPU 都需要单独配置。
...
:::info GPU 调度与列表更新
当有多个 GPU 可用时，Olares 可能会将运行中的应用分配到不同的 GPU 上。切换 GPU 模式或恢复应用后，应用与 GPU 的分配关系可能不会立刻更新到界面中。

如果某个运行中的应用暂时未出现在当前 GPU 的列表中，请先等待几秒，再点击对应区域中的 <i class="material-symbols-outlined">sync</i> 刷新列表。系统通常会在 GPU 调度完成后自动更新分配结果。
