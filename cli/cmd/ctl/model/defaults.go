...
// `olares-cli model default …` — which model answers when a caller names none.
//
...
Examples:
  ...
  olares-cli model default show --system
`,
		Args: cobra.NoArgs,
...
func renderResolvedDefaults(ctx context.Context, pc *preparedClient, w io.Writer, items []resolvedDefault) error {
	if len(items) == 0 {
		_, err := fmt.Fprintln(w, "no default resolves for any kind of request, so a request that names no "+
			"model is refused. Attach a model to a provider first — `olares-cli model list` shows what exists.")
		return err
	}
...
func renderSystemDefaults(ctx context.Context, pc *preparedClient, w io.Writer, items []systemDefault) error {
	if len(items) == 0 {
		_, err := fmt.Fprintln(w, "the workspace has no defaults set, so every blank request falls back to "+
			"the oldest enabled model of its kind. Set one with `olares-cli model default set`.")
		return err
	}
	names := modelNames(ctx, pc)
...
alone when only one provider offers it. "model list" shows all three.
...
Examples:
  olares-cli model default set --mode chat=claude/claude-sonnet-4-5
  olares-cli model default set --mine --mode chat=lmstudio/qwen3-8b
  olares-cli model default set --mode chat=<id> --mode embedding=<id>
`,
		Args: cobra.NoArgs,
		RunE: func(c *cobra.Command, _ []string) error {
...
Examples:
  olares-cli model default clear --mine chat
  olares-cli model default clear chat embedding
`,
...
func findModelForMode(known []adminModelRow, ref, mode string) (*adminModelRow, error) {
	...
	if len(matches) == 0 {
		return nil, fmt.Errorf("no model %q; `olares-cli model list` shows what is configured", ref)
	}
	if len(matches) > 1 {
	...
	if !found.Model.Enabled {
		return nil, fmt.Errorf("%s/%s is not offered to callers, so a request falling back to it would fail; "+
			"enable it with `olares-cli model provider models update %s %s --enable`",
