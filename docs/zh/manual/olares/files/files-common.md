...
| --- | --- | --- | --- |
| **存储** | 按用户隔离个人文件。 | ❌ 否。文件仅限<br>单个用户访问。 | ✅ 是。多个应用可以<br>访问某用户的个人文件。 |
| **应用 > 数据** | 按应用隔离数据。 | ✅ 是。某个应用的所有用户共享其数据。 | ❌ 否。文件仅限单个<br>应用访问。 |
| **应用 > 缓存** | 按应用隔离临时缓存。 | ✅ 是。某个应用的所有用户共享其缓存。 | ❌ 否。文件仅限单个<br>应用访问。 |
| **应用 > 公共** | 跨用户、跨应用共享文件。 | ✅ 是。所有用户访问<br>相同的文件。 | ✅ 是。所有应用访问相同的文件。 |
...
公共目录通过提供跨用户、跨应用的共享存储解决上述问题，让不同应用和用户访问相同的模型文件，避免重复存储。
