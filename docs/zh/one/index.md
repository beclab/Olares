---
description: Olares One 的简要介绍，以及它与 Olares OS 的集成，用于本地 AI 处理。
aside: false
head:
  - - meta
    - name: keywords
      content: Olares One, Olares, 个人 AI 云, 本地 AI, 边缘计算, 数据隐私
---

# Olares One 文档

Olares One 是一款专为个人 AI 云打造的专属硬件设备。它将工作站级的强劲硬件与 Olares OS 系统结合，让你能够在本地流畅运行各类 AI 工具并安全存放数据。从此，你无需再将个人文件、隐私数据和提示词（Prompts）上传至云端服务器，真正实现对数字资产的完全掌控。

通过 Olares One，你可以轻松实现以下核心场景：

* **边缘 AI**：在本地运行开源 AI 模型，极速生成文本或图音视频内容，确保所有数据绝对“不出端”。
* **畅玩游戏**：借助 Steam Headless 应用，将设备瞬间化身高性能游戏主机，在 Olares One 或其他兼容设备上尽情游玩。
* **个人知识库**：让 AI 深度读取并理解你的私人文档，为你量身打造随叫随到的专属数字助手。
* **文件存储**：通过自带的 Files 应用，轻松实现个人文件的一站式集中管理、多端同步与安全分享。
* **私有工作空间**：一键在本地部署专属的办公与团队协作套件，打造高效、私密的数字办公环境。
* **私有媒体库**：搭配 Jellyfin 等应用搭建个人影视中心，随时随地串流播放你的海量电影与音乐收藏。

:::info
本文档仅聚焦于 Olares One 的专属硬件使用场景。如需了解通用的系统功能或探索更多玩法，请点击顶部导航栏中的 **Olares OS** 或 **Use cases** 标签。
:::

## 开启你的 Olares One 之旅

请跟随以下指南，从首次开机起步，一步步解锁从日常基础使用到极客高级玩法的完整体验。

<div class="vpath">
  <div class="vpath-stage">
    <div class="vpath-marker"><span class="vpath-icon"><span class="material-symbols-outlined">power_settings_new</span></span></div>
    <div class="vpath-body">
      <div class="vpath-title">初始设置</div>
      <div class="vpath-links">
        <a href="./first-boot">首次启动</a>
        <a href="./access-olares-via-vpn">通过 VPN 访问 Olares</a>
        <a href="./access-olares-via-local-domain">通过 .local 域名访问 Olares</a>
        <a href="./redeem-membership">兑换会员</a>
      </div>
    </div>
  </div>
  <div class="vpath-stage">
    <div class="vpath-marker"><span class="vpath-icon"><span class="material-symbols-outlined">waving_hand</span></span></div>
    <div class="vpath-body">
      <div class="vpath-title">快速上手</div>
      <div class="vpath-links">
        <!-- <a href="./open-webui">与本地 LLM 聊天</a> -->
        <a href="./olares-onboarding">通过自然语言管理 Olares</a>
        <!-- <a href="./comfyui">使用 ComfyUI 生成图像和视频</a> -->
        <a href="./gpu">切换 GPU 模式</a>
      </div>
    </div>
  </div>
  <div class="vpath-stage">
    <div class="vpath-marker"><span class="vpath-icon"><span class="material-symbols-outlined">apps</span></span></div>
    <div class="vpath-body">
      <div class="vpath-title">使用</div>
      <div class="vpath-links">
        <a href="./customize">定制 Olares</a>
        <a href="./files">管理文件</a>
        <a href="./backup-restore">备份和恢复数据</a>
        <a href="./market">安装和更新应用</a>
        <a href="./vault">保护密码</a>
        <!-- <a href="./wise-download">下载 YouTube 视频</a> -->
        <div class="vpath-seealso">另见：<a href="../manual/olares/">Olares OS 手册中的所有系统应用指南</a></div>
      </div>
    </div>
  </div>
  <div class="vpath-stage">
    <div class="vpath-marker"><span class="vpath-icon"><span class="material-symbols-outlined">manage_accounts</span></span></div>
    <div class="vpath-body">
      <div class="vpath-title">管理</div>
      <div class="vpath-links">
        <a href="./create-users">添加和管理用户</a>
        <a href="./config-app-access">配置应用访问</a>
      </div>
    </div>
  </div>
  <div class="vpath-stage">
    <div class="vpath-marker"><span class="vpath-icon"><span class="material-symbols-outlined">monitoring</span></span></div>
    <div class="vpath-body">
      <div class="vpath-title">监控</div>
      <div class="vpath-links">
        <a href="./dashboard">系统资源</a>
        <a href="./space">流量</a>
      </div>
    </div>
  </div>
  <div class="vpath-stage">
    <div class="vpath-marker"><span class="vpath-icon"><span class="material-symbols-outlined">explore</span></span></div>
    <div class="vpath-body">
      <div class="vpath-title">探索</div>
      <div class="vpath-links">
        <a href="./steam-direct-play">玩 Steam 游戏</a>
        <a href="./windows">在 Olares 中访问 Windows</a>
        <a href="./ace-step">使用 ACE-Step 生成音乐</a>
        <div class="vpath-seealso">另见：<a href="../use-cases/">浏览所有使用场景</a></div>
      </div>
    </div>
  </div>
  <div class="vpath-stage">
    <div class="vpath-marker"><span class="vpath-icon"><span class="material-symbols-outlined">build</span></span></div>
    <div class="vpath-body">
      <div class="vpath-title">高级</div>
      <div class="vpath-links">
        <a href="./access-overview">访问 Olares One 终端</a>
        <a href="./expand-storage-usb-drive">扩展存储</a>
        <a href="./connect-two-olares-one">连接两台 Olares One</a>
        <a href="./egpu">搭配 eGPU 使用</a>
        <a href="./dual-boot-dual-drive">与 Windows 双系统启动</a>
        <a href="./dual-boot-ubuntu-dual-drive">与 Ubuntu 双系统启动</a>
        <a href="./install-windows-primary-drive">在 Olares One 上安装 Windows</a>
        <a href="./install-ubuntu-server">在 Olares One 上安装 Ubuntu</a>
      </div>
    </div>
  </div>
  <div class="vpath-stage">
    <div class="vpath-marker"><span class="vpath-icon"><span class="material-symbols-outlined">update</span></span></div>
    <div class="vpath-body">
      <div class="vpath-title">系统更新</div>
      <div class="vpath-links">
        <a href="./update">更新系统</a>
        <a href="./factory-reset">恢复 Olares One</a>
      </div>
    </div>
  </div>
  <div class="vpath-stage">
    <div class="vpath-marker"><span class="vpath-icon"><span class="material-symbols-outlined">menu_book</span></span></div>
    <div class="vpath-body">
      <div class="vpath-title">参考</div>
      <div class="vpath-links">
        <a href="./spec">技术规格</a>
        <a href="./faq">常见问题</a>
        <a href="./known-issues">已知问题</a>
      </div>
    </div>
  </div>
</div>
