...
| [共享应用](../../../developer/concepts/application.md#共享应用) | 一种特殊类型的社区应用，为集群内所有用户提供共享的资源或服务。<br>管理员安装后，所有成员均可直接使用；后端类服务则通过共享入口<br>暴露标准 API，供第三方客户端调用。 |
...
此类共享应用仅提供 API 服务，不包含图形界面。任何支持对应 API 的客户端均可调用。以 Ollama 为例：

1. **管理员安装**：管理员在应用市场安装 Ollama。安装后，共享服务在集群内启动，并暴露标准 API 访问入口。
2. **成员配置使用**：

    a. 获取访问地址：在 Olares **设置** > **应用** > **Ollama** > **共享入口**中查看。
    
    b. 配置客户端：安装任意支持 Ollama API 的第三方客户端，如 LobeChat、Open WebUI，并在客户端配置中填入上述共享入口地址，即可开始使用。  
