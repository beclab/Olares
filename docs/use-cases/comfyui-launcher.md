...
| Entry | Location in Files | Description |
 -- | -- | -- |
| **Root** | `External > olares > ai > comfyui > ComfyUI` | Root directory of the ComfyUI installation. |
| **Plugin** | `External > olares > ai > comfyui > ComfyUI > custom_nodes` | Stores installed plugins (custom nodes and extensions).                    |
| **Model**  | `External > olares > ai > model` | Centralized storage for AI models shared by ComfyUI and other AI apps.    |
| **Output** | `External > olares > ai > output > comfyui` | Default destination for images and other assets generated by ComfyUI.     |
| **Input**  | `External > olares > ai > comfyui > ComfyUI > input`  | Folder for source images used in image-to-image or in-painting workflows. |
