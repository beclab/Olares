...
// ----------------------------------------------------------------------------
// unknownSubcommandRunE — typo guard for dispatch-only parents.
//
// The dashboard tree's parents (dashboard, applications, overview gpu) are
// pure dispatch nodes; without a RunE they fell through to cobra's default
// help text on a typo, with exit 0. unknownSubcommandRunE restores the
// "Did you mean…" suggestion + non-zero exit. These tests pin both halves
// of the contract so a future cobra upgrade that changes default behaviour
// surfaces here loudly.
// ----------------------------------------------------------------------------

// newTypoFixture builds a (root → parent w/ unknownSubcommandRunE) tree that
// matches the real olares-cli wiring (dashboard is a subcommand of the
// olares-cli root, never the root itself). The shape matters: cobra's default
// args validator (legacyArgs in cobra v1.9 args.go:24) takes a hard
// "unknown command" path when an unknown positional lands on a command that
// has subcommands AND no parent — so testing on a bare root would bypass our
// RunE altogether. See cobra args.go:28-39.
...
// TestBuildGPUDetailFullEnvelope_PartialFailure: pin the soft-failure
// contract — a single instant query 5xx must NOT abort the envelope.
// We expect:
//   - the failed gauge's `raw.error` populated with the upstream
//     message (or our wrapper);
//   - env.Meta.Warnings[0] mentioning the gauge key + the error;
//   - the other 5 gauges, all 4 trends, and the detail section all
//     present and non-empty.
func TestBuildGPUDetailFullEnvelope_PartialFailure(t *testing.T) {
	var instantCalls int
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch {
		// gpuAdvisory side-effects: silence them with admin
		// stub + a CUDA-bearing node so no stderr advisory line
		...
		case strings.HasSuffix(r.URL.Path, "/instant-vector"):
			instantCalls++
			body, _ := io.ReadAll(r.Body)
			// The util_core instant query is the only one whose
			// PromQL string contains the literal `hami_core_util{`
			// fragment (sibling queries embed `hami_core_util_avg`
			// or other counters). Fail that one with 502 to
			// exercise the per-gauge soft-failure path.
		...
		default:
			// Silently 200 anything else (e.g. capability probes).
			_, _ = w.Write([]byte(`{}`))
		}
	}))
	defer srv.Close()
...
// gauges; CLI must do the same. Today both branches happen to emit
// the same two usage gauges (allocation gauges aren't injected as
// extra rows yet), so the gauge count stays at 2 in both modes — the
// regression net here is symbolic: it captures the wiring intent and
// fails loudly if a future "always emit allocation gauges" refactor
// breaks the contract.
