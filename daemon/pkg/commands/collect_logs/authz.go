...
func authorize(ctx context.Context, kubeClient kubernetes.Interface, p *Param) (*resolvedScope, error) {
	username, role := p.CallerUsername, p.CallerRole
	if username == "" {
		return nil, errors.New("collect-logs: missing caller identity")
	}

	rs := &resolvedScope{username: username, role: role}
	privileged := role == utils.Owner || role == utils.Admin
	var denied []string
