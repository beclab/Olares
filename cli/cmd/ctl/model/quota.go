...
// `olares-cli model quota …` — the ceilings Router enforces.
//
...
func findModelByQualifiedName(ctx context.Context, pc *preparedClient, ref string) (*adminModelRow, error) {
	...
	return nil, fmt.Errorf("no model %q; `olares-cli model list` shows what is configured, "+
...
Examples:
  ...
  olares-cli model quota list --user pptest01
  olares-cli model quota list -o json
`,
		Args: cobra.NoArgs,
...
func renderQuotaList(ctx context.Context, pc *preparedClient, w io.Writer, rows []quotaRow, target *quotaTarget) error {
	if len(rows) == 0 {
		...
		_, err := fmt.Fprintf(w, "%s. `olares-cli model quota set --user <name> --budget 25` is the shape of one.\n", what)
		return err
	}
...
Examples:
  olares-cli model quota set --user pptest01 --budget 25
  olares-cli model quota set --key ci --rpm 60 --tpm 120000
  olares-cli model quota set --model "openai/gpt-4o" --rpm 10 --warn-at 90
`,
		Args: cobra.NoArgs,
		RunE: func(c *cobra.Command, _ []string) error {
			...
			for kind, v := range want {
				if v <= 0 {
					return fmt.Errorf("--%s must be greater than zero; "+
						"`olares-cli model quota clear` removes a ceiling instead", flagForQuotaKind(kind))
				}
			}
			var pct *int
...
Examples:
  olares-cli model quota clear --key ci --yes
  olares-cli model quota clear --user pptest01 --budget
`,
		Args: cobra.NoArgs,
		RunE: func(c *cobra.Command, _ []string) error {
