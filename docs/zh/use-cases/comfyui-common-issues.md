...
Olares 1.12.6 对[共享应用](/zh/manual/olares/market/shared-apps)系统进行了重大更新。因此，你无法直接更新 ComfyUI，而需要安装基于新架构的新版 ComfyUI 应用。只需按照下方的迁移步骤操作，你的所有数据（包括模型、插件、工作流和文件）都会自动迁移。

你现有的 `ComfyUI 共享版` 在升级到 1.12.6 后仍可继续使用，但无法接收后续更新。我们建议你尽快迁移到新版本。

### 迁移步骤

1. 卸载当前的 `ComfyUI 共享版`。在弹窗中，**不要**勾选 "同时删除所有本地数据"。
2. 打开应用市场，搜索 **ComfyUI**，点击**安装**。

   在应用详情页查看 **信息** 下的 **兼容性** 项，若显示为 **"Olares >=1.12.6-0"**，说明安装的是新版本。
...
:::warning
迁移完成后，请勿再将模型或输入文件上传至 `External/<your_hostname>/ai/` 目录。新版 ComfyUI 不再挂载这些目录，因此无法识别放置其中的文件。
:::

数据迁移会在每次 ComfyUI 重启时进行。如果旧路径下新增了文件，将在下次重启时迁移到新位置，并删除 `External/<your_hostname>/ai/` 下的原始文件。为避免文件冲突和迁移延迟，请使用新目录上传文件。
