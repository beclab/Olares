...
When uploading external models manually or selecting a destination in **Custom Download**, it is important to place files into the correct subfolder. The same structure is shared across Olares AI applications:

```plain
/Files/External/olares/ai/model
  ├─ main/               → Base checkpoints (SD 1.5, SDXL, Flux)
  ├─ vae/                → VAE models
  ├─ lora/               → LoRA weights
  ├─ controlnet/         → ControlNet models
  ├─ ipadapter/          → IP-Adapter models
  ├─ upscale_models/     → Upscalers (ESRGAN, SwinIR, etc.)
  ├─ clip_vision/        → CLIP Vision models (used for IP-Adapter)
  ├─ unet/               → Standalone UNet weights (Flux, SD3)
  ├─ text_encoders/      → Text encoder models (T5, CLIP-L/G)
  ├─ clip/               → CLIP models
  ├─ embeddings/         → Textual Inversion embeddings
  ├─ hypernetworks/      → Hypernetwork weights
  ├─ facerestore_models/ → Face restoration (GFPGAN, CodeFormer)
  ├─ inpaint/            → Inpainting-specific models
  ├─ style_models/       → Style or effect models
  └─ diffusion_models/   → Other diffusion backbones
```
Make sure the model type matches the target folder so that ComfyUI (and other AI apps) can detect and use the files correctly.
...
Essential models are basic resources required for ComfyUI to run, including SD large models, VAE, preview decoders, and auxiliary tool models. We recommend installing them on first run.
...
| Status | Switch version | Install | Enable | Disable | Uninstall |
| -- | -- | -- | -- | -- | -- 
| `Not installed` | ✔ | ✔  | –  | – | – |
| `Installed`     | ✔ | – | – | ✔ | ✔ |
| `Disabled`      | ✔ | – | ✔ | – | ✔ |
