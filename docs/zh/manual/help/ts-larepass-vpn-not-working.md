...
当 LarePass 专用网络在 macOS、Windows 或移动设备上无法正常使用时，可参考本指南进行排查。适用场景包括：专用网络开关无响应、专用网络状态一直停留在“连接中”、之前可用的专用网络连接突然停止，或专用网络显示“连接中”后自动关闭。
...
**macOS**
- 点击 LarePass 桌面客户端的专用网络连接开关没有反应，或专用网络连接停滞在“连接中”。
- LarePass 专用网络之前在此设备上可以正常使用，但现在无法连接或连接后立即断开。
 
**Windows**
- 点击 LarePass 桌面客户端的专用网络开关没有反应，或专用网络无法启用。

### 原因

- **macOS**：LarePass 专用网络需要完整设置系统级网络扩展和 VPN 配置。如果在初次设置时跳过或未完成任一步骤，或者网络扩展出现卡死或损坏，macOS 将阻止 LarePass 建立专用网络隧道。

- **Windows**：第三方杀毒或安全软件可能误将 LarePass 桌面客户端标记为可疑程序，导致专用网络服务无法启动。
...
开启专用网络连接后，状态会停留在“连接中”一段时间，随后专用网络连接自动关闭。

### 原因

LarePass 客户端设备的系统时间可能不正确，导致专用网络握手失败。
