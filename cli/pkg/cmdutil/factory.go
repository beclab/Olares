...
func (f *Factory) ValidAccessToken(ctx context.Context) (string, error) {
	...
	// rp.AuthURL was derived from rp.Location, so the refresh must ride the
	// same position's transport. An unprobed profile falls back to external,
	// matching sharedLocationState.
	loc := rp.Location
	if !loc.Valid() {
		loc = olares.LocationExternal
	}
	newAT, rerr := f.Refresher().Refresh(ctx, rp.OlaresID, rp.AuthURL, tok, rp.InsecureSkipVerify, loc)
	if rerr != nil {
		return "", rerr
	}
	cell.update(newAT)
