...
| `Permission denied` / EACCES writing data, or data not persisting | uid != 1000 on userspace mounts (root-owned dirs, missing `runAsUser`) | Chart fix: run identity / uid 1000 |
| Admission denied: untrusted image runs as root | OPA blocks a root third-party main container | Chart fix: force uid 1000 / initContainer chown |
