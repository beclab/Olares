...
// is not.
// cliURL and wizardURL are where to fetch the release from, and are passed in
// rather than read here because the upgrade target that names them lives on
// the control node. Empty means the CDN.
func PrepareNode(ctx context.Context, version, cliURL, wizardURL string) error {
	v, err := semver.NewVersion(version)
	if err != nil {
		return fmt.Errorf("invalid target version %q: %v", version, err)
	}
	target := state.UpgradeTarget{Version: *v, CliURL: cliURL, WizardURL: wizardURL}
