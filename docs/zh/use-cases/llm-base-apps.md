...
:::warning
WARNING 本页面由 AI 自动翻译，部分技术术语可能与中文习惯存在差异。如有疑问，请以英文原文为准。
:::
...
1. 打开 Market，搜索 "引擎基座"。
    ...
    ![Market 中的引擎基座应用](/images/manual/olares/llm-base-apps.png#bordered)
...
1. 在 **Configure environment variables for [New-app-name]** 窗口中，根据目标模型和引擎填写以下信息。
    ...
    | **ENGINE_ARGS** | 设置引擎启动参数。上下文长度（`-c`）为必填项。多个参数之间用空格分隔。<br><br>示例：<ul><li>`-c 65536`</li><li>`-c 65536 -ngl all`</li></ul>更多参数见[引擎调优参数](#reference-engine-tuning-arguments)。 |
    ...
    | **MODEL_MODE** | 选择 **Chat** 或 **Embedding**。 |
    | **MODEL_SUPPORTS** | 选择模型支持的能力：**Vision**、**Tools** 或 **Thinking**。若为嵌入模型，请选择 **None**。 |
    | **ENGINE_ARGS** | 设置引擎启动参数。上下文长度为必填项。多个参数之间用空格分隔。<br><br>示例：<ul><li>`OLLAMA_CONTEXT_LENGTH=8192`</li><li>`OLLAMA_CONTEXT_LENGTH=8192 OLLAMA_KV_CACHE_TYPE=q8_0`</li></ul>更多参数见[引擎调优参数](#reference-engine-tuning-arguments)。 |
    ...
    | **MODEL_MODE** | 选择 **Chat** 或 **Embedding**。 |
    | **MODEL_SUPPORTS** | 选择模型支持的能力：**Vision**、**Tools** 或 **Thinking**。若为嵌入模型，请选择 **None**。 |
    | **ENGINE_ARGS** | 设置引擎启动参数。上下文长度为必填项。多个参数之间用空格分隔。<br><br>示例：<ul><li>vLLM：`--max-model-len 65536`</li><li>SGLang：`--context-length 65536`</li></ul>更多参数见[引擎调优参数](#reference-engine-tuning-arguments)。 |
...
| `OLLAMA_KEEP_ALIVE` | 设置最后一次请求后模型在 GPU 显存中的驻留时长。到期后权重会被换出到系统内存。<ul><li>`-1` 表示永久驻留在 GPU 显存中。</li><li>`3m` 表示驻留 3 分钟。</li></ul>默认值：`-1`。 | `-1` 或 `30m` |
| `OLLAMA_FLASH_ATTENTION` | 启用 Flash Attention。使用 `OLLAMA_KV_CACHE_TYPE` 进行 KV 缓存量化时必须开启。<br><br>默认值：`1`。 | `1`（开启） |
...
| `--mem-fraction-static` | 设置预分配给静态用途的显存比例，类似 vLLM 的 `--gpu-memory-utilization`。 | `0.85` |
| `--chunked-prefill-size` | 将超长输入分块处理，避免长时间占用 GPU，让并发请求的流式输出保持顺畅。 | `4096` |
...
| `--max-num-batched-tokens` | 限制单个批次处理的最大 token 数，使遇到超长请求时响应时间保持稳定。 | `8192` |
...
6. 在 **GPU residency** 部分，点击 **Detect**，然后：
    ...
    - **查看模式**：确认模型是否运行在 GPU 上。
        - `Full GPU`：整个模型运行在 GPU 上。这是预期中最快的状态。
        - `Split` 或 `CPU`：模型的一部分或全部回退到了 CPU，会让推理明显变慢。在 GPU 主机上，这通常意味着环境挂载有误；请检查 `{ENGINE}_REQUIRED_GPU_MEMORY` 设置和引擎参数。
...
5. 运行一个任务来测试连接。本示例使用 Olares skill 将一个应用上传并部署到 Olares。
    ...
    在本示例中，Todo 应用被上传并部署到了 **My Olares**。从 **My Olares** 或 Launchpad 打开它即可使用这个运行中的应用。

    ![部署到 My Olares 的 Todo 应用](/images/manual/olares/llm-base-model-inst-task.png#bordered)

## 卸载模型实例

1. 打开 Market，进入 **My Olares**，然后找到该模型实例应用。
2. 点击操作按钮旁的下拉箭头，然后选择 **Uninstall**。
