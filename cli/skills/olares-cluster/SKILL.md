...
| Live-watch a workload's rollout (`readyReplicas` converging to `replicas`) | Phase 3 (continued) — `cluster workload rollout-status --watch` is not implemented yet. Today, `cluster workload get -w` would also work as a manual fallback only after that flag lands; for now poll `cluster workload get` by hand. |
| Live-watch all workloads / pods in an application namespace as a single dashboard | Phase 3 (continued) — `cluster application status [--watch]` is not implemented yet. Today: combine `cluster application workloads <ns>` and `cluster application pods <ns>` in a manual loop. |
...
| [`cli/cmd/ctl/cluster/pod/`](cli/cmd/ctl/cluster/pod) | `cluster pod` verbs (`list`, `get` with `--watch`, `yaml`, `events`, `logs`); exports `pod.Get` + `pod.SplitNsName` + `pod.RunList` + `pod.RunLogs` + `pod.LogsOptions` for sibling packages. |
