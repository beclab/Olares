...
func runShareInternal(
	...
	tgt, err := frontendPathToShareTarget(pathArg, share.TypeInternal)
	if err != nil {
		return err
	}
	...
	// repo name when constructing the share's display label and
	// echoing the path back to the user. Failure here is non-fatal
	// (lookupSyncRepoName returns "" on any error), and an empty
	// name falls through to the legacy "extend as name" behavior
	// so a flaky /api/repos/ endpoint never blocks a share-create.
	syncRepoName := ""
	if tgt.FileType == "sync" {
		syncRepoName = lookupSyncRepoName(ctx, f, tgt.Extend)
	}

	res, err := client.Create(ctx, tgt, share.CreateOptions{
		Name:       shareNameFromPath(tgt, syncRepoName),
	...
	fmt.Fprintf(out, "  path      : %s\n", shareTargetDisplay(tgt, syncRepoName))
...
`,
	...
	addOutputFormatFlag(cmd, &o.asJSON, "print the created share as JSON instead of a summary")
	return cmd
}
...
func runSharePublic(
	...
	// frontendPathToShareTarget runs the per-flavor namespace gate
	// (see [validateShareNamespace] in share.go) BEFORE the
	// volume-listing-layer / node-picker-layer checks, so a Public
	// share against external/<node>/ surfaces the broader "Public
	// only supports drive" error rather than the narrower
	// volume-listing-layer one — same final answer for the user
	// but a more actionable explanation.
	tgt, err := frontendPathToShareTarget(pathArg, share.TypePublic)
	if err != nil {
		return err
	}
	...
	// Public shares are gated to the `drive` namespace by the
	// flavor allow-list, so syncRepoName is structurally always
	// the empty string here — but keep the lookup symmetric with
	// the other create flavors for a consistent invariant
	// (shareNameFromPath / shareTargetDisplay both no-op cleanly
	// on non-sync paths).
	syncRepoName := ""
	if tgt.FileType == "sync" {
		syncRepoName = lookupSyncRepoName(ctx, f, tgt.Extend)
	...
	// The generated password is in here for the same reason it is in
	// the human output: the server does not echo it back on any later
	// read, so a caller who did not capture it now cannot recover it.
	if o.asJSON {
		return writeShareJSON(out, struct {
			*share.Result
			...
			Result:            res,
			Password:          password,
			PasswordGenerated: autoGenerated,
			DisplayPath:       shareTargetDisplay(tgt, syncRepoName),
			LinkTemplate:      "<share-host>/sharable-link/" + res.ID + "/",
		})
	}
...
func runShareSMB(
	...
	tgt, err := frontendPathToShareTarget(pathArg, share.TypeSMB)
	if err != nil {
		return err
	}
	...
	// SMB excludes the sync namespace (matches the LarePass GUI's
	// per-driver gating), so syncRepoName is structurally always
	// the empty string here — same symmetry rationale as runSharePublic.
	syncRepoName := ""
	if tgt.FileType == "sync" {
		syncRepoName = lookupSyncRepoName(ctx, f, tgt.Extend)
	}
