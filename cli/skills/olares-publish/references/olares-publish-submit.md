...
6. Confirm every credential the app needs has a source outside the committed files, since the whole folder becomes public in `beclab/apps`. Each one is supplied by the user at install (`envs[]` with `type: password`), owned by Olares middleware, or generated by the chart at render time — the three recipes are in [`../../olares-chart/references/olares-chart-secrets.md`](../../olares-chart/references/olares-chart-secrets.md). To confirm the folder carries none of its own:
