...
```
olares-cli model local status  llamacppqwen3627bggufv3
olares-cli model local progress llamacppqwen3627bggufv3 --watch
```
...
olares-cli model local spec show llamacppqwen3627bggufv3 -o json > card.json
olares-cli model local spec set llamacppqwen3627bggufv3 --from card.json
```
...
```
olares-cli model local retry   <app>   # re-enter download and load now
olares-cli model local restart <app>   # relaunch the engine with the current card
```
...
If both leave the phase where it was, the problem is below the console — an image that will not pull, a node without the memory, a GPU binding that is missing. Continue in [deciding which layer is wrong](olares-model-diagnosis.md), which routes to [`olares-doctor`](../../olares-doctor/SKILL.md) for those.
