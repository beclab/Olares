...
type upgradeHarness struct {
	...
	// startFailsLeft is how many more times Start on this node should fail
	// with a transport error before answering. Distinct from unreachable:
	// that one also fails the readiness probe, so the upgrade never reaches
	// dispatch.
	startFailsLeft map[string]int
	// startAlwaysFails keeps Start on this node returning a transport error.
	startAlwaysFails map[string]bool
	// startAttempts counts Start calls per node, including the ones that
	// failed.
	...
	// goingDown, by node name, makes that node's stage record say the machine
	// is on its way down — what a node whose GPU driver changed reports.
	goingDown map[string]bool
...
func newUpgradeHarness(t *testing.T, plan UpgradePlan, nodes []inventory.Node) *upgradeHarness {
	...
	start := func(node string, req UpgradeStageRequest) (UpgradeStageState, error) {
		h.mu.Lock()
		defer h.mu.Unlock()
		h.startAttempts[node]++
		if h.startAlwaysFails[node] {
			return UpgradeStageState{}, errors.New("node did not answer")
		}
		if n := h.startFailsLeft[node]; n > 0 {
			h.startFailsLeft[node] = n - 1
			return UpgradeStageState{}, errors.New("node did not answer")
		}
		...
		state := &UpgradeStageState{
			...
			RestartPending: h.goingDown[node],
		}
		if state.RestartPending {
			// The boot it ran on, which is what the control node proves a
			// restart against.
			state.BootID = ranOn
		}
		if h.failAt[key] {
...
func (h *upgradeHarness) attempts(node string) int {
	h.mu.Lock()
	defer h.mu.Unlock()
	return h.startAttempts[node]
}
...
// The upgrade target names the run. Once it is gone the version is gone too,
// and GetByRequest has nothing to look up. ActiveOfType is what still finds
// the run so it can be stopped.
func TestTheActiveUpgradeCanBeFoundWithoutItsRequestID(t *testing.T) {
	h := newUpgradeHarness(t, samplePlan(), twoNodeCluster())
	h.deps.Store = newTestStore(t)
	...
	got, ok := m.ActiveOfType(TypeUpgrade)
	if !ok {
		t.Fatal("no active upgrade, want the one just started")
	}
	if got.ID != op.ID {
		t.Errorf("active upgrade = %s, want %s", got.ID, op.ID)
	}
	if _, ok := m.ActiveOfType(TypeShutdown); ok {
		t.Error("reported a shutdown while an upgrade was running")
	}

	if !m.RequestStop(got.ID) {
		t.Fatal("the run ActiveOfType named refused to be stopped")
	}
	settled := waitForTerminal(t, m, op.ID)
	if settled.Code != CodeUpgradeCancelled {
		t.Fatalf("settled as %s/%s, want %s", settled.Status, settled.Code, CodeUpgradeCancelled)
	}
	if _, ok := m.ActiveOfType(TypeUpgrade); ok {
		t.Error("still reported an active upgrade after it settled")
	}
}
...
//
// It is given a moment first — see awaitNodeReady, and the resumed runs that
// meet a worker on its way back rather than broken. This node never comes
// back, so the moment is made a short one.
func TestUpgradeRefusesANotReadyNode(t *testing.T) {
	nodes := twoNodeCluster()
	nodes[0].Ready = false
	h := newUpgradeHarness(t, samplePlan(), nodes)
	h.deps.Timeouts = Timeouts{Poll: time.Millisecond, Ready: 20 * time.Millisecond}
...
// A node that misses the first few dispatches is retried inside the same
// operation, so a worker whose olaresd is still coming back from the previous
// stage does not fail the whole upgrade.
func TestUpgradeRetriesABrieflyUnreachableDispatch(t *testing.T) {
	h := newUpgradeHarness(t, samplePlan(), twoNodeCluster())
	h.startFailsLeft["worker-1"] = 3
	...
	if got := h.attempts("worker-1"); got <= 3 {
		t.Fatalf("Start on worker-1 called %d times, want more than the 3 failures", got)
	}
}

// A node that never answers is still failed, after the stage deadline, not
// on the first missed POST.
func TestUpgradeGivesUpWhenDispatchNeverAnswers(t *testing.T) {
	h := newUpgradeHarness(t, samplePlan(), twoNodeCluster())
	h.startAlwaysFails["worker-1"] = true

	now := time.Now()
	h.deps.Now = func() time.Time { return now }
	h.deps.Sleep = func(context.Context, time.Duration) error {
	...
	if got := h.attempts("worker-1"); got < 2 {
		t.Fatalf("Start on worker-1 called %d times, want at least one retry", got)
	}
}
...
func TestARebootStageFailsANodeThatNeverReturns(t *testing.T) {
	...
	h := newUpgradeHarness(t, plan, twoNodeCluster())
	// Both nodes are up when the stage starts — a node about to be rebooted
	// has to be, or there is no boot to prove it left. worker-1 says it is
	// going down, then leaves the cluster's view and never comes back;
	// master-1 has nothing to reboot for.
	h.observed = map[string]inventory.Observation{
		"worker-1": {BootID: "boot-a", Ready: true},
		"master-1": {BootID: "boot-a", Ready: true},
	}
	h.goingDown["worker-1"] = true
	h.onStart = func(node, stage string) {
		if node != "worker-1" || stage != "02-all-nodes" {
			return
		}
		h.mu.Lock()
...
func TestASerialRebootStageWaitsBeforeTheNextNode(t *testing.T) {
	...
	// Only worker-1 reboots. master-1 runs the same stage and says nothing is
	// coming, so it is not waited for at all — what is being asserted is that
	// it does not even start until worker-1 is back.
	h.goingDown["worker-1"] = true
	h.deps.Timeouts = Timeouts{Poll: time.Millisecond, Stage: time.Minute,
...
// A resumed upgrade waits for the cluster to come back rather than failing
// because it is not there yet.
//
// The control node reboots as part of the upgrade, and systemd brings olaresd
// up alongside k3s rather than after it. On a real two-worker cluster the
// resumed run asked for the node directory 34ms after starting, while k3s was
// still printing "Starting", got inventory_unavailable, and failed the whole
// operation — and because the version had already been flipped by then, the
// watcher removed the upgrade target instead of retrying, so the upgrade ended
// quietly on a failure nobody looked at.
func TestAResumedUpgradeWaitsForTheClusterToComeBack(t *testing.T) {
	h := newUpgradeHarness(t, samplePlan(), twoNodeCluster())
	h.deps.Store = newTestStore(t)

	// Unreadable for the first few asks, the way an apiserver that is still
	// starting is.
	var mu sync.Mutex
	attempts := 0
	nodes := twoNodeCluster()
	h.deps.Inventory = func(context.Context) ([]inventory.Node, error) {
		mu.Lock()
		defer mu.Unlock()
		attempts++
		if attempts <= 3 {
	...
	settled := waitForTerminal(t, m, op.ID)
	if settled.Status != StatusSucceeded {
		t.Fatalf("status = %s (%s: %s), want the upgrade to have waited out the restart",
			settled.Status, settled.Code, settled.Error)
	}
	...
	defer mu.Unlock()
	if attempts < 4 {
		t.Errorf("the node directory was asked for %d times, want it retried until readable", attempts)
	}
}

// It does not wait for ever: a cluster that never comes back still settles.
func TestAnUpgradeGivesUpOnAClusterThatNeverComesBack(t *testing.T) {
	h := newUpgradeHarness(t, samplePlan(), twoNodeCluster())
	h.deps.Store = newTestStore(t)
	h.deps.Timeouts = Timeouts{Poll: time.Millisecond, Stage: time.Minute,
		Down: time.Millisecond, Ready: 20 * time.Millisecond}
	h.deps.Inventory = func(context.Context) ([]inventory.Node, error) {
		return nil, errors.New("connection refused")
	}
	...
	settled := waitForTerminal(t, m, op.ID)
	if settled.Code != CodeInventoryUnavailable {
		t.Errorf("settled as %s/%s, want %s", settled.Status, settled.Code, CodeInventoryUnavailable)
	}
}

// A reboot stage does not wait for machines that are not rebooting.
//
// The GPU driver upgrade reboots only the nodes whose driver it changed, so on
// most releases most nodes run this stage and go nowhere. Before the node
// reported what it was doing there was no telling those from a node that had
// simply not begun going down yet, and each one cost a full Down timeout,
// serially: measured at three minutes a node on a real cluster, nine minutes
// for three nodes that all stayed up.
//
// Down here is far longer than the test could tolerate, which is the point —
// if any node is waited out, this does not finish.
func TestARebootStageDoesNotWaitForNodesThatAreNotRebooting(t *testing.T) {
	plan := samplePlan()
	for i := range plan.Stages {
		if plan.Stages[i].Name == "02-all-nodes" {
			plan.Stages[i].AwaitRestart = true
	...
	// Nothing is going down: no node sets goingDown.
	h.deps.Timeouts = Timeouts{Poll: time.Millisecond, Stage: time.Minute,
		Down: 30 * time.Second, Ready: time.Minute}
	h.deps.Store = newTestStore(t)
	...
	if waited := time.Since(started); waited > 5*time.Second {
		t.Errorf("the stage took %s with nothing to reboot, want it not to wait out Down (%s) per node",
			waited, h.deps.Timeouts.Down)
	}
}

// The node is what decides, so a node that says a restart is coming is waited
// for even though every other node in the same stage is not.
func TestARebootStageWaitsForTheNodeThatSaysItIsGoingDown(t *testing.T) {
	plan := samplePlan()
	for i := range plan.Stages {
		if plan.Stages[i].Name == "02-all-nodes" {
			plan.Stages[i].AwaitRestart = true
	...
	h.goingDown["worker-1"] = true
	h.deps.Timeouts = Timeouts{Poll: time.Millisecond, Stage: time.Minute,
		Down: time.Second, Ready: 5 * time.Second}
	h.deps.Store = newTestStore(t)
	...
	defer mu.Unlock()
	if !cameBack {
		t.Error("the stage finished without waiting for the node that said it was rebooting")
	}
}

// The release locations reach the nodes being prepared.
//
// Only the control node has an upgrade target, so only it knows the operator
// named a private mirror instead of the CDN. A worker told just the version
// downloads from the CDN, which on an air-gapped or mirrored install is a
// machine that cannot be reached — the upgrade works on the control node and
// fails on every other one.
func TestTheReleaseLocationsReachTheNodesBeingPrepared(t *testing.T) {
	h := newUpgradeHarness(t, samplePlan(), twoNodeCluster())
	h.deps.Store = newTestStore(t)
	h.deps.Upgrade.Locations = func() UpgradeLocations {
		return UpgradeLocations{
			CliURL:    "https://mirror.internal/olares-cli.tar.gz",
			WizardURL: "https://mirror.internal/wizard.tar.gz",
		}
	}

	var mu sync.Mutex
	seen := map[string]UpgradeStageRequest{}
	inner := h.deps.Upgrade.Start
	h.deps.Upgrade.Start = func(ctx context.Context, node inventory.Node,
		req UpgradeStageRequest, token string) (UpgradeStageState, error) {
		mu.Lock()
		seen[req.Stage] = req
		mu.Unlock()
		return inner(ctx, node, req, token)
	}
	...
	mu.Lock()
	defer mu.Unlock()
	prepare, ok := seen[StageNodePrepare]
	if !ok {
		t.Fatal("no node was asked to prepare itself")
	}
	if prepare.CliURL != "https://mirror.internal/olares-cli.tar.gz" {
		t.Errorf("prepare carried cliURL %q, want the operator's mirror", prepare.CliURL)
	}
	if prepare.WizardURL != "https://mirror.internal/wizard.tar.gz" {
		t.Errorf("prepare carried wizardURL %q, want the operator's mirror", prepare.WizardURL)
	}

	// Nothing else downloads a release, so nothing else is told where one is.
	for stage, req := range seen {
		if stage == StageNodePrepare {
			continue
		}
		if req.CliURL != "" || req.WizardURL != "" {
			t.Errorf("stage %s was sent release locations it has no use for", stage)
		}
	}
}

// A daemon wired without the seam still upgrades: no override means the CDN,
// which is where every node looked before there was anywhere else to look.
func TestAnUpgradeWithoutNamedLocationsStillRuns(t *testing.T) {
	h := newUpgradeHarness(t, samplePlan(), twoNodeCluster())
	h.deps.Store = newTestStore(t)
	h.deps.Upgrade.Locations = nil
...
// A node already back on another boot is not waited for again.
//
// This is what a resumed run meets: reboot-nodes took the control node down,
// and the orchestrator that comes back re-runs the stage against a record
// saying a restart was pending. It was, once. Proving it against the boot the
// node recorded rather than against an observation this run made — it has none
// from before its own start — is what lets it see the restart already
// happened and move on instead of waiting out a boot that is in the past.
func TestANodeAlreadyBackOnAnotherBootIsNotWaitedForAgain(t *testing.T) {
	plan := samplePlan()
	for i := range plan.Stages {
		if plan.Stages[i].Name == "02-all-nodes" {
			plan.Stages[i].AwaitRestart = true
	...
	h := newUpgradeHarness(t, plan, twoNodeCluster())
	h.deps.Store = newTestStore(t)
	// Both nodes are up, on a boot later than the one the stage ran on.
	h.observed = map[string]inventory.Observation{
		"worker-1": {BootID: "boot-b", Ready: true},
		"master-1": {BootID: "boot-b", Ready: true},
	}
	// worker-1 already ran the stage, on the boot before, and said it was
	// going down. It has since done so and come back.
	h.stageStates[stageKey("worker-1", "02-all-nodes")] = &UpgradeStageState{
		OperationID: "", Stage: "02-all-nodes", Phase: UpgradeStagePhaseSucceeded,
		RestartPending: true, BootID: "boot-a",
	}
	// Long enough that waiting for anything would outlast the test.
	h.deps.Timeouts = Timeouts{Poll: time.Millisecond, Stage: time.Minute,
		Down: 30 * time.Second, Ready: time.Minute}
	...
	if waited := time.Since(started); waited > 5*time.Second {
		t.Errorf("took %s, want the restart recognised as already done rather than waited out (Down is %s)",
			waited, h.deps.Timeouts.Down)
	}
}

// And the other half: a node that said it was going down and is back but not
// yet Ready is still waited for, because a serialised reboot stage may not
// take the next machine down while this one is still coming up.
func TestANodeBackButNotReadyIsStillWaitedFor(t *testing.T) {
	plan := samplePlan()
	for i := range plan.Stages {
		if plan.Stages[i].Name == "02-all-nodes" {
			plan.Stages[i].AwaitRestart = true
	...
	h.onStart = func(node, stage string) {
		...
		// Back on a new boot almost at once, but not Ready for a while after.
		h.mu.Lock()
		h.observed["worker-1"] = inventory.Observation{BootID: "boot-b", Ready: false}
		h.mu.Unlock()
		go func() {
	...
	defer mu.Unlock()
	if !becameReady {
		t.Error("the stage released the slot while the rebooted node was back but not yet Ready")
	}
}

// A node that said it was going down and then never did fails the stage.
//
// The reboot command is issued without its error being read, so a machine can
// report the restart as owed and never take it. Treating that as done would
// release a serialised stage onto the next machine and finish the upgrade over
// a node whose new GPU driver is installed and not loaded — the exact state
// the reboot exists to get out of.
func TestANodeThatPromisedARestartAndStayedUpFailsTheStage(t *testing.T) {
	plan := samplePlan()
	for i := range plan.Stages {
		if plan.Stages[i].Name == "02-all-nodes" {
			plan.Stages[i].AwaitRestart = true
	...
	// It says a restart is coming and then stays exactly where it is.
	h.goingDown["worker-1"] = true
	h.deps.Timeouts = Timeouts{Poll: time.Millisecond, Stage: time.Minute,
		Down: 30 * time.Millisecond, Ready: 5 * time.Second}
	...
	settled := waitForTerminal(t, m, op.ID)
	if settled.Status == StatusSucceeded {
		t.Fatal("the upgrade succeeded over a reboot that never happened")
	}

	step := stepNamed(t, settled, "02-all-nodes")
	var found bool
	for _, n := range step.Nodes {
		if n.NodeName == "worker-1" && n.Code == CodeNodeDidNotGoDown {
			found = true
		}
	}
	if !found {
		t.Errorf("worker-1 is not recorded as having stayed up: %+v", step.Nodes)
	}
	// The stage is serialised, so nothing else may have been reached.
	for _, key := range h.order() {
		if key == "master-1/02-all-nodes" {
			t.Error("the next machine was started after a reboot went missing")
		}
	}
}

// A worker that is briefly NotReady does not fail a resumed upgrade.
//
// The stages this upgrade runs are themselves what makes a node NotReady for a
// moment — the container runtime restarts in one, k3s in another — so an
// orchestrator killed during either comes back to a worker on its way back
// rather than a broken one. Refusing there fails an operation whose later
// stages were already done, and the retry starts a new one that runs the whole
// upgrade again.
func TestATransientlyNotReadyWorkerDoesNotFailTheUpgrade(t *testing.T) {
	nodes := twoNodeCluster()
	nodes[0].Ready = false // the directory caught it mid-restart
	h := newUpgradeHarness(t, samplePlan(), nodes)
	h.deps.Store = newTestStore(t)
	h.observed = map[string]inventory.Observation{
		"worker-1": {BootID: "boot-a", Ready: false},
		"master-1": {BootID: "boot-a", Ready: true},
	}
	h.deps.Timeouts = Timeouts{Poll: time.Millisecond, Stage: time.Minute,
		Down: time.Second, Ready: 5 * time.Second}
	...
	go func() {
		...
		h.mu.Lock()
		h.observed["worker-1"] = inventory.Observation{BootID: "boot-a", Ready: true}
		h.mu.Unlock()
	}()
	...
	if got := waitForTerminal(t, m, op.ID); got.Status != StatusSucceeded {
		t.Fatalf("status = %s (%s: %s), want the worker's return to have been waited for",
			got.Status, got.Code, got.Error)
	}
}
