...
name: olares-shared
version: 1.2.0
description: "Olares profile and authentication foundation for olares-cli — required prerequisite for every other olares-cli skill (files, market, settings, dashboard, cluster). Covers the Olares profile model (one profile = one Olares instance + one olaresId), first-time login with password and optional TOTP, importing an existing refresh_token, switching / listing / removing Olares profiles, OS-keychain token storage, automatic access_token refresh on 401/403, and the full Olares auth-error recovery table. Use when the user mentions Olares, olares-cli, OpenClaw on Olares, profile, olaresId, login, logout, two-factor / 2FA / TOTP, refresh token, keychain, or sees errors like 'server rejected the access token', 'refresh token for X became invalid', 'no access token for X', 'already authenticated', or 'two-factor authentication required'."
metadata:
  requires:
    bins: ["olares-cli"]
