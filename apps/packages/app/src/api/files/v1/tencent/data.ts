...
export default class TencentDataAPI extends DriveDataAPI {
	...
	formatPathtoUrl(path: string): string {
		return `${path}?src=${DriveType.Tencent}`;
	}
	...
	async copy(el: FileItem, type: string): Promise<CopyStoragesType> {
		const from = this.breadcrumbsBase + decodeURIComponent(el.path).slice(14);
	...
	async deleteItem(items: FileItem[]): Promise<void> {
		...
		for (let i = 0; i < items.length; i++) {
			const item = items[i];
			promises.push(remove(item.path + '?src=' + DriveType.Tencent));
		}
	...
	async createDir(dirName: string, path: string): Promise<void> {
		...
		console.log('createDir newName', dirName);

		let url =
			path + '/' + encodeURIComponent(dirName) + '?src=' + DriveType.Tencent;
		url = url.replace('//', '/');
		console.log('createDir url', url);
		...
		if (['text', 'txt', 'textImmutable'].includes(file.type)) {
			try {
				const res = await this.commonAxios.get(
					`/api/raw${file.path}?src=${DriveType.Tencent}`,
					{}
				);
				file.content = res;
			} catch (error) {
