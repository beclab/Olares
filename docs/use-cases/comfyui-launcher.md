...
|--|--|--|
| **Root** | `/Files/Data/comfyuisharev3/`<br /> `comfyui/ComfyUI` | Root directory of the ComfyUI installation. |
| **Plugin** | `/Files/Data/comfyuisharev3/comfyui/`<br /> `ComfyUI/custom_nodes` | Stores installed plugins (custom nodes and extensions). |
| **Model**  | `/Files/Common/comfyui/model` | Centralized storage for AI models shared by ComfyUI and other AI apps. |
| **Output** | `/Files/Common/comfyui/output` | Default destination for images and other assets generated by ComfyUI. |
| **Input**  | `/Files/Common/comfyui/input` | Folder for source images used in image-to-image or in-painting workflows. |

Clicking any of these entries opens the Files app directly in the corresponding directory.

:::info File locations used before Olares 1.12.6
Earlier versions of ComfyUI used these locations:

- Root directory: `/Files/External/olares/ai/comfyui/ComfyUI`
- Models: `/Files/External/olares/ai/model`
- Output files: `/Files/External/olares/ai/output/comfyui`
- Input files: `/Files/External/olares/ai/comfyui/ComfyUI/input`

When the new ComfyUI is installed, it migrates supported data to the new locations automatically.
:::

### Understand the `model` directory structure

ComfyUI in Olares uses a file structure that differs from the standard installation. This change allows models to be shared between ComfyUI and other applications.
...
|--|--|--|
| `checkpoints/` | `checkpoints/` | Base checkpoints (SD 1.5, SDXL, Flux) |
...
:::tip For users who installed ComfyUI before Olares 1.12.6
In earlier versions of ComfyUI, the `checkpoints/` folder was named `main/`. The new version of ComfyUI will automatically rename the `main/` folder to `checkpoints/` during migration.
:::

### Configure additional model paths

You normally do not need to edit `extra_model_paths.yaml`. Add a mapping only when a model is already stored under `/Files/Common/comfyui/model/`, but ComfyUI or a custom node cannot find it.

#### About the configuration

ComfyUI uses `extra_model_paths.yaml` to map model categories to folders outside its default model directory. On Olares, this file is generated automatically and allows ComfyUI to load models from the shared model directory.

The configuration file is located at `/Files/Data/comfyuisharev3/comfyui/ComfyUI/user/extra_model_paths.yaml`.

The shared model directory has two path representations:

- In Files: `/Files/Common/comfyui/model/`
- Inside ComfyUI: `/mnt/olares-shared-model`

The generated configuration starts with a section similar to the following:
...
Each entry under `olares_shared_models` maps a model category to a folder relative to `base_path`:

```yaml
<model category>: <folder relative to base_path>
```

- The key on the left is the category that ComfyUI or a custom node looks up.
- The value on the right is the folder under `/Files/Common/comfyui/model/` where models in that category are stored.

The model category must match the value expected by ComfyUI or the custom node. Do not invent a category name. Use the exact category specified in the node documentation or error message.

For example, suppose a custom node looks for the `detection` category and the model is stored at `/Files/Common/comfyui/model/detection/mediapipe_face_fp32.safetensors`.

Add the following mapping under `olares_shared_models`:
```yaml
olares_shared_models:
  base_path: /mnt/olares-shared-model
  # Existing mappings...
...
This tells ComfyUI to search the following folder for models in the `detection` category: `/Files/Common/comfyui/model/detection/`.

The category and folder path do not need to match. For example:
```yaml
olares_shared_models:
  base_path: /mnt/olares-shared-model
  # Existing mappings...
...
```
This tells a custom node that requests the `ultralytics_bbox` category to search `/Files/Common/comfyui/model/ultralytics/bbox/`.

#### Add a mapping

1. Open Files and go to `Data/comfyuisharev3/comfyui/ComfyUI/user/`.
2. Open `extra_model_paths.yaml`.
3. Under `olares_shared_models:`, check whether the required category already exists, then add or update the mapping using the same indentation as the existing entries.
4. Save the file and restart ComfyUI.
5. Reopen the affected workflow or node and confirm that the model appears in the expected selector.

If the model still does not appear, check the model category, relative folder path, YAML indentation, and model file location.
...
| **Use from Library** | Missing models where you already<br> have a compatible alternative installed. | Uses models from `/Common/comfyui/model/`. No download required, but you need to verify compatibility yourself. |
...
To verify or manage downloaded files, open Files and go to the corresponding subfolder under `/Common/comfyui/model/`.
...
**Use from Library** lets you select models from `/Common/comfyui/model/`. The button is available only when the corresponding model-type folder exists and contains model files.
...
3. Navigate to `/Common/comfyui/model/`.
...
4. Open Files from the Launchpad and go to `/Data/comfyuisharev3/comfyui/`.
