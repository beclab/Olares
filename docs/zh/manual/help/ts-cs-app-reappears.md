...
# 无法在应用独占模式移除已暂停应用 <Badge type="warning" text="仅适用于 Olares 1.12.5" />

:::info 适用版本
此问题及解决方案仅适用于 Olares 1.12.5。如果使用 Olares 1.12.6 或更高版本，可参阅[从 AI 算力资源中移除应用](../olares/settings/gpu-resource.md#从-ai-算力资源中移除应用)。
:::

在**应用独占**模式下，如果无法移除已暂停的应用，导致无法将 GPU 分配给其他应用，请参考本指南。 

## 适用情况

- 在**应用独占**模式下，暂停并移除某个应用后，该应用仍出现在**选择独占应用**区域。
