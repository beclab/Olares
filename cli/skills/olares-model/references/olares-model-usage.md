...
| Question | Record |
|---|---|
| What was called, by whom, and what did it cost? | `model usage` — one row per model call |
| Who changed Router, and to what? | `model audit` — one row per management write |
| What did an agent framework do around a call? | `model trace` — the spans it reported |

## Usage

```
olares-cli model usage summary --since 7d
olares-cli model usage summary --by user --since 30d
olares-cli model usage list --status failed --limit 20
olares-cli model usage export --since 30d --out calls.csv
```
...
```
olares-cli model audit list --since 24h
olares-cli model audit list --target-type provider --failed
olares-cli model audit get <id>
```
...
```
olares-cli model trace list --since 24h
olares-cli model trace get <trace-id>
olares-cli model trace capture
```

A trace is what an agent framework reported around a call — the spans, their timings, and optionally the prompt and completion content. Nothing appears here unless something instrumented sent it; an ordinary `model call` produces a usage row and no trace.
...
If the whole `trace` subtree answers 404, this Router has observability switched off and stores no traces at all. That is a deployment setting, not a missing row, and `model usage` still works — one row per call rather than the spans around it.
