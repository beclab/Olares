...
# Set up OpenCode as your AI coding agent — Olares 1.12.6
...
For multi-local-model setups under [oh-my-openagent](opencode-omo-1.12.6.md), use single-model apps. Each model gets its own endpoint, which lets you register it as a separate provider and tune per-model settings like concurrency limits.
:::
...
- [Orchestrate multi-agent workflows with oh-my-openagent](opencode-omo-1.12.6.md): Enable OMO to run multi-agent collaboration in OpenCode.
