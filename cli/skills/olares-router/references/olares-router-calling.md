...
**Translation.** `translate` translates, `--detect` identifies a language instead, and `--languages` lists the pairs the configured model serves. `--to` is required for a translation and `--from` is optional, since detection is the default. `--transcript <file>` is the fifth route and the one that is not a text at a time: it translates a conversation as a stretch, so the model reads the turns around each line, and answers one result per turn under the ids that were sent — never shorter, so nothing shifts onto the wrong turn. Use it for anything a diarizer or a meeting tool produced; `--per-line` on the same file translates each turn with nothing around it, which is how a two-word reply comes back as "yes" when it meant "right". The file is a `segments` array or an object carrying one alongside `context`, `background` and `glossary`; 64 turns is the ceiling, and splitting means carrying the tail of each part into the next one's `context`. `--background` and repeatable `--term source=target` add the same two things from the command line. A turn the model failed on comes back carrying its reason, in place, so the rest of the page is still usable.
...
**`listen` takes samples, not a file format.** There is no decoder on the other end, so any container handed to it — wav, mp3, m4a — is transcribed as noise and reported as an empty transcript at exit 0. Produce raw PCM, and make `--sample-rate` match what you produced:
...
**An empty result at exit 0 is not evidence the model heard nothing.** These verbs succeed and return nothing for several unrelated reasons, so cross-check before reporting one as a finding: `call vad <file>` says whether there is speech at all, `call transcribe <file>` whether it is intelligible, and `xxd <file> | head -1` whether the bytes are the format you think they are. An empty diarization in particular is most often a single-speaker recording rather than silence — a real 16 kHz recording of one person produced no turns while `vad` found two speech segments in the same file.
