...
| [共享应用](shared-apps.md) | 一种特殊类型的社区应用，由管理员统一部署，为集群内所有<br>用户提供共享的资源或服务。带界面的应用可在启动台直接打开使用；<br>无界面的后端服务则暴露标准 API，供客户端连接使用。 |
...
启动应用市场后，默认进入**发现**页面。该页面作为你的核心探索枢纽，通过直观的版块分类引导你探索各类内容：
...
1. 从程序坞或启动台，打开**应用市场**。
2. 找到目标应用，双击查看其详情。
3. 如果该应用支持多种硬件加速器，在**资源**区域配置部署信息：

    ![加速器](/images/zh/manual/olares/market-accelerator1.png#bordered)

    a. 从下拉列表中选择所需的计算资源。例如 **NVIDIA GPU**、**NVIDIA GB10** 或 **CPU**。

    b. 查看所选计算资源对应的 **CPU**、**内存**、**所需磁盘空间**和**显存**要求，确保当前硬件配置符合要求。

4. 点击**获取**。
5. 当操作按钮变为**安装**时，点击**安装**。
6. 如果弹出提示，点击**确认**以确认你的硬件加速器选择。安装开始。
7. （可选）若要取消安装，点击按钮右侧的 <i class="material-symbols-outlined">close_small</i>。
8. 安装完成后，按钮将变为**打开**。

### 安装共享应用

共享应用由管理员统一部署，集群成员无需自行安装。为确保共享服务在集群内正常运行和可访问，请根据共享应用的类型采取相应的安装流程。

:::info 共享应用的管理
管理员负责共享应用的升级、暂停、恢复、卸载。这些操作会影响集群内所有成员，操作前请确认。
:::

#### 无界面的后端服务应用

此类共享应用仅提供 API 服务，不包含图形界面。任何支持对应 API 的客户端均可调用。以基于 **Ollama 引擎基座** 创建的模型实例为例：

1. **管理员部署**：管理员在应用市场中基于 **Ollama 引擎基座**创建模型实例。部署完成后，模型实例作为共享服务在集群内启动，并在启动台生成一个同名的模型应用入口。
2. **成员配置使用**：

    a. 获取访问地址：在启动台打开模型应用，进入模型控制台，复制其中显示的 **Base URL**。

    b. 配置客户端：安装任意支持对应 API 的客户端（如 LobeChat、Open WebUI），在客户端配置中填入上述地址即可使用。

#### 自带用户界面的完整应用

此类共享应用同时包含后端服务与 Web 用户界面，可独立为用户提供服务，如 Dify 共享版、ComfyUI 共享版等。
    
1. **管理员安装**：管理员在应用市场安装共享应用。安装后，集群内会启用共享服务，并在启动台添加一个同名的应用入口。
    ::: tip ComfyUI 启动器
    ComfyUI 共享版包含一个桌面启动器组件，用于管理 ComfyUI 服务和相关资源。管理员需要从 ComfyUI 启动器里配置并启用服务。
    :::
2. **成员直接使用**：成员在启动台找到该应用入口，点击打开即可直接使用，无需安装额外客户端。

### 安装自定义应用

1. 准备 Olares 应用 Chart 文件（格式为 `.zip`、`.tgz`、`.tar` 或 `.gz`）。
2. 打开**应用市场**，从左侧菜单进入**我的 Olares**。
