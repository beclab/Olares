...
// `olares-cli model list` — GET /console/api/provider-models
// `olares-cli model capabilities` — GET /console/api/capabilities/supports
//
...
this answer — Router keeps them out of the aggregate list. "model provider get
...
Examples:
  ...
  olares-cli model list --provider claude --enabled
  olares-cli model list --search qwen -o json
`,
		Args: cobra.NoArgs,
...
func renderModelList(w io.Writer, items []adminModelRow, total, limit, offset int) error {
	if len(items) == 0 {
		_, err := fmt.Fprintln(w, "no models match. If nothing at all is configured, start with "+
			"`olares-cli model provider types` to see what can be added.")
		return err
	}
...
// `model capabilities` is the vocabulary for --supports. It is a fixed list
