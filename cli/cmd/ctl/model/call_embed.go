...
rather than guessing. "olares-cli model list --mode embedding" shows the ones
that qualify.

Examples:
  olares-cli model call embed "the quick brown fox"
  olares-cli model call embed "first" "second" "third"
  cat lines.txt | olares-cli model call embed --per-line -o json
  olares-cli model call embed "text" --model bge-m3 --dimensions 512
`,
