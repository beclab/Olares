...
如果问题仍未解决，或者想讨论文档中未列出的硬件组合，可以前往 [Olares 论坛](https://www.olares.com/forum/)发帖。请提供以下信息，方便其他人了解并复现你的使用场景：

```text
【系统】Windows 或 Olares OS
【硬件】显卡扩展坞型号 + 显卡型号，例如 AOOSTAR EG02 + RTX 4060 Ti
【连接方式】Olares One、雷电 5 线材、扩展坞如何连接，中间是否经过其他雷电扩展坞或 hub
【开机顺序】例如：扩展坞通电 → 连接线材 → 启动 Olares One
【现象】例如：卡在 Olares logo、nvidia-smi 找不到 eGPU，或运行 10 分钟后掉卡
【发生概率】例如：尝试 5 次，3 次失败、2 次正常
【已尝试的操作】例如：已安装并启用 Gen1 临时方案，或已更换线材
【附件】Olares OS 请附上 egpu-report-*.txt，Windows 请附上前文收集的截图和错误信息
```
