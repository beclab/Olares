...
:::info GPU scheduling and list updates
When multiple GPUs are available, Olares may distribute running apps across different GPUs. After you switch GPU modes or resume an app, the app-to-GPU assignment may not appear in the UI immediately.

If a running app does not appear on the current GPU, wait a few seconds, then click <i class="material-symbols-outlined">sync</i> in that section to refresh the list. In most cases, Olares updates the assignment automatically after GPU scheduling is complete.
