...
// `olares-cli model audit …` — who changed Router, and to what.
//
...
// and what it cost is `model usage`.
...
Model calls are elsewhere: "olares-cli model usage" is what was called and what
it cost.
...
Examples:
  ...
  olares-cli model audit list --actor pptest01 --failed
  olares-cli model audit list --action provider.update -o json
`,
		Args: cobra.NoArgs,
...
func renderAuditList(w io.Writer, items []auditEntry, total, offset int, merged bool) error {
	...
	_, err := fmt.Fprintln(w, "\n`olares-cli model audit get <id>` carries what the change altered.")
	return err
}
...
Examples:
  olares-cli model audit get 0f3a1c62-...
  olares-cli model audit get 0f3a1c62-... -o json
`,
...
func runAuditGet(ctx context.Context, f *cmdutil.Factory, id, outputRaw string) error {
	...
	if id == "" {
		return fmt.Errorf("an audit entry id is required; `olares-cli model audit list` has them in the ID column")
	}
	pc, err := prepare(ctx, f)
	if err != nil {
		return err
