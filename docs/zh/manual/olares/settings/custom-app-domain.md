...
3. 在**端点配置**中，你可以看到 Jellyfin 的默认路由 ID，通常为一串字母和数字的组合。
4. 在**设置自定义路由 ID** 旁，点击 <i class="material-symbols-outlined">add</i>。
5. 输入一个更容易记忆和识别的路由 ID，例如 `jellyfin`。
6. 点击**确认**。

   ![自定义路由 ID](/images/zh/manual/olares/custom-route-id1.png#bordered){wdith=90%}
   
现在，你可以通过新的 URL 访问 Jellyfin：`https://jellyfin.alexmiles.olares.com`。
