...
默认情况下，`base_path` 会使 `/Common/comfyui/model/` 下的所有顶层子目录对 ComfyUI 可用。但是，嵌套子目录中的模型（例如 `/Common/comfyui/model/ultralytics/bbox/face_yolov8m.pt`）可能无法被自动检测。此时需要添加显式的子目录映射。

例如，要注册 `detection` 子目录：
...
要为使用非标准目录的自定义节点注册路径（例如 ImpactPack 的 bbox YOLO 模型使用的 `ultralytics_bbox`）：
