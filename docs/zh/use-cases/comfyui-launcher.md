...
ComfyUI的早期版本中的文件位置不同：
应用根目录 `/Files/External/<your-hostname>/ai/comfyui/`，模型目录 `/Files/External/<your-hostname>/ai/model/`，输入输出目录`/Files/External/<your-hostname>/ai/output/comfyui/`
:::

在 1.12.6 或更高版本安装新版的 ComfyUI 后，数据将存储在上述目录并支持在 AI 应用之间共享。如果您之前安装过早期版本，新版 ComfyUI 会自动将数据从旧的位置迁移到新的位置。
...
ComfyUI 使用名为 `extra_model_paths.yaml` 的配置文件来定位默认 ComfyUI 目录以外的模型。在 Olares 中，该文件会自动生成，并配置为指向 `Common` 共享模型库。

默认配置如下：
- **`base_path`**：指向 `Common/comfyui/model`，即所有 AI 应用共享的模型目录。
- **模型子目录**：上文 `model` 表中列出的模型子目录

#### 何时需要自定义

如果工作流无法识别 `Common/comfyui/model/` 中的模型，通常是因为模型所在的子目录未被注册到 `extra_model_paths.yaml` 配置文件。只有被注册的子目录才会被 ComfyUI 自动扫描。

例如，已上传模型文件 `Common/comfyui/model/detection/mediapipe_face_fp32.safetensors`，但由于 `detection` 子目录未在配置中声明，因此需要在 `extra_model_paths.yaml` 中添加如下条目。这里的键（`detection`）对应 ComfyUI 需要识别的模型类别或节点搜索路径，值为 `detection`，表示实际`Common/comfyui/model/`下的文件夹名称。
...
编辑方法：
1. 打开 Files 并导航到`/Data/comfyuisharev3/comfyui/user/extra_model_paths.yaml`
2. 在文本编辑器中打开 `extra_model_paths.yaml`。
3. 根据需要添加或修改路径。参见上方示例。
