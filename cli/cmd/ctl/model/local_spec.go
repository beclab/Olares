...
// `model local spec` — GET/PUT /api/model-spec, GET /api/model-spec/file
// `model local config` — GET /api/config
// `model local endpoints` — GET /api/endpoints
//
...
Examples:
  olares-cli model local spec file llamacppqwen3627bggufv3
  olares-cli model local spec file llamacppqwen3627bggufv3 > card.json
`,
		Args: cobra.ExactArgs(1),
		RunE: func(c *cobra.Command, args []string) error {
			ctx := c.Context()
...
Examples:
  ...
  cat card.json | olares-cli model local spec set llamacppqwen3627bggufv3
`,
		Args: cobra.ExactArgs(1),
		RunE: func(c *cobra.Command, args []string) error {
...
func runLocalSpecSet(ctx context.Context, f *cmdutil.Factory, ref, from string, yes bool, outputRaw string) error {
	...
	if p := strings.TrimSpace(li.Provider); p != "" {
		_, err = fmt.Fprintf(os.Stdout, "\nWritten. Router reads this on its next sync; "+
			"`olares-cli model provider sync-models %q` does it now.\n", p)
		return err
	}
	_, err = fmt.Fprintf(os.Stdout, "\nWritten. Router does not have this application as a provider, so "+
		"nothing routes to it yet — `olares-cli model provider register %s` is what adds it.\n", li.AppName)
	return err
}
...
Examples:
  olares-cli model local config llamacppqwen3627bggufv3
  olares-cli model local config llamacppqwen3627bggufv3 -o json
`,
		Args: cobra.ExactArgs(1),
		RunE: func(c *cobra.Command, args []string) error {
			ctx := c.Context()
...
Examples:
  olares-cli model local endpoints llamacppqwen3627bggufv3
  olares-cli model local endpoints llamacppqwen3627bggufv3 --unavailable
`,
		Args: cobra.ExactArgs(1),
		RunE: func(c *cobra.Command, args []string) error {
			ctx := c.Context()
