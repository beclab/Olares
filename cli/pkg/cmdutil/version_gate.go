...
// Version gates are the shared way command trees express an Olares-backend
// version requirement, so individual areas don't re-roll the same
// OlaresBackendAtLeast + error-message boilerplate.
//
...
func RejectIfRemoved(ctx context.Context, f *Factory, gate RemovedGate) error {
	if f == nil {
		return nil
	}
