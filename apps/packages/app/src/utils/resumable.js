...
(function () {
	...
	var Resumable = function (opts) {
		...
		$.defaults = {
			...
			permanentErrors: [
				400, 401, 403, 404, 409, 415, 440, 441, 442, 443, 500, 501
			],
			maxFiles: undefined,
		...
		var appendFilesFromFileList = function (fileList, event) {
			...
			$h.each(fileList, function (file) {
				var isIgnoreFile = $.getOpt('isIgnoreFile');
				if (typeof isIgnoreFile === 'function') {
					if (isIgnoreFile(file, event)) {
						return;
					}
				}
				...
				if ($.isPaused() === false) {
					...
					$h.each($.chunks, function (chunk) {
						if (chunk.status() == 'pending' && chunk.preprocessState !== 1) {
							if (chunk.fileObjSize == 0 && chunk.createEmptyFileEnable()) {
								chunk.createEmptyFile();
							} else {
								chunk.send();
							}
		...
		function ResumableChunk(resumableObj, fileObj, offset, callback) {
			...
			$.send = async function () {
				...
				const chunkMd5Func = $.resumableObj.getOpt('chunkMd5');
				let chunkMd5 = '';
				if (typeof chunkMd5Func === 'function') {
					chunkMd5 = await chunkMd5Func(bytes);
				}
				if (chunkMd5 && chunkMd5.length > 0) {
					query['md5'] = chunkMd5;
				}
				...
				if ($.pendingRetry) {
					...
					if ($.xhr.status == 200 || $.xhr.status == 201) {
						...
						const uploadSuccessCode =
							$.resumableObj.getOpt('uploadSuccessCode');
						if (typeof uploadSuccessCode === 'function') {
							const result = uploadSuccessCode($.xhr);
							if (typeof result === 'boolean') {
								if (!result && $.retries < $.getOpt('maxChunkRetries')) {
									$.abort();
									return 'pending';
								}
								return result ? 'success' : 'error';
							}
							return result.includes($.xhr.status) ? 'success' : 'error';
						} else if (typeof uploadSuccessCode === 'object') {
							return uploadSuccessCode.includes($.xhr.status)
								? 'success'
								: 'error';
						}
			...
			$.createEmptyFile = async function () {
				var createEmptyFile = $.getOpt('createEmptyFile');
				if (typeof createEmptyFile === 'function') {
					$.preprocessState = 1;
					const result = await createEmptyFile($.fileObj);
					if (result) {
						$.preprocessState = 2;
						$.markComplete = true;
			...
			$.createEmptyFileEnable = () => $.getOpt('createEmptyFile') != undefined;
			return this;
		}
