export default {
	...
	devices: '设备',
	forcing_vpn_access_to_private_network: '强制通过 VPN 访问',
	...
	'errors.password_cannot_contain_spaces': '密码不能包含空格',
	...
	Internal: '内部',
	'VRAM mode': '显存模式',
	'Change GPU mode to': '将 GPU 模式切换为{mode}？',
	'Changing the GPU mode will reallocate resources. Apps using this GPU may stop and need to be resumed manually before they can run again.':
		'切换 GPU 模式将重新分配资源。当前使用该 GPU 的应用可能会暂停，需要手动恢复后才能继续运行。',
	...
	from_space_url: '从 Space 链接恢复',
	from_aws_s3_url: '从 AWS S3 链接恢复',
	'New folder name': '新文件夹名称',
	from_tencent_cos_url: '从 Tencent COS 链接恢复',
	...
	'Multiple apps take turns using the same GPU': '多个应用轮流使用同一块 GPU',
	'Manage assigned apps': '管理已分配应用',
	'Assign an app to this GPU': '将应用分配到此 GPU',
	'Multiple apps share a single GPU, with fixed VRAM allocated to each app':
		'多个应用共享同一块 GPU，每个应用分配固定显存',
	'Allocate VRAM': '分配显存',
	'Set how much VRAM each app can use': '设置每个应用可使用的显存',
	'A single app has exclusive access to the GPU': '单个应用独占整个 GPU',
	...
	'Select the video encoding that we should transcode to. We will use software encoding when hardware acceleration for the selected format is not available. H264 encoding will always be enabled.':
		'选择要转码的视频编码格式。当所选格式不支持硬件加速时，将使用软件编码。H264 编码将始终启用。',
	...
	'Select the maximum number of threads to use when transcoding. Reducing the thread count will lower CPU usage but may not convert fast enough for a smooth playback experience.':
		'选择转码时使用的最大线程数。减少线程数会降低 CPU 使用率，但可能导致转码速度不足而影响播放流畅度。',
	Automatic: '自动',
	Max: '最大',
	...
	Folder: '文件夹',
	'Enable VBR audio encoding': '启用 VBR 音频编码',
	...
	'Algorithm used to downmix multi-channel audio to stereo.':
		'用于将多声道音频降混为立体声的算法。',
	'H.265 encoding CRF': 'H.265 编码 CRF',
	'H.264 encoding CRF': 'H.264 编码 CRF',
	...
	"The 'Constant Rate Factor' (CRF) is the default quality setting for the x264 and x265 software encoders. You can set the values between 0 and 51, where lower values would result in better quality (at the expense of higher file sizes). Sane values are between 18 and 28. The default for x264 is 23, and for x265 is 28, so you can use this as a starting point. Hardware encoders do not use these settings.":
		'恒定码率因子（CRF）是 x264 和 x265 软件编码器的默认质量设置。取值范围为 0 到 51，数值越低质量越好（但文件体积越大）。合理范围通常在 18 到 28 之间，x264 默认值为 23，x265 默认值为 28，可作为初始设置参考。',
	...
	'Maximum number of packets that can be buffered while waiting for all streams to initialize. Try to increase it if you still meet "Too many packets buffered for output stream" error in FFmpeg logs. The recommended value is 2048.':
		'等待所有流初始化时可缓冲的最大数据包数。若 FFmpeg 日志出现"输出流缓冲包过多"错误可尝试增加该值，推荐值为2048',
	'Low-power encoding saves unnecessary CPU-GPU synchronization. The i915 HuC firmware must be configured correctly before use in Linux.':
		'低功耗编码减少不必要的 CPU-GPU 同步，Linux 系统需先正确配置 i915 HuC 固件',
	"Prefer the system's native DXVA or VA-API hardware decoder":
		'优先使用系统原生 DXVA 或 VA-API 硬件解码器',
	'Enable Enhanced NVDEC Decoder': '启用增强型 NVDEC 解码器',
	'Enable low-power Intel HEVC hardware encoder':
		'启用低功耗 Intel HEVC 硬件编码器',
	'Enable low-power Intel H.264 hardware encoder':
		'启用低功耗 Intel H.264 硬件编码器',
	...
	'Import cookie': '导入 cookie',
	'No cookie found': '没有发现 cookie',
	...
	'Add and manage custom hosts rules to easily optimize your network access and DNS resolution.':
		'添加并管理自定义主机规则，轻松优化你的网络访问和 DNS 解析。',
	'Add hosts': '添加主机',
	'No apps assigned': '暂无已分配应用',
	'Assign app': '分配应用',
	'Switch app': '切换应用',
	Operation: '操作',
	...
	'The total space your files currently occupy on your device.':
		'待备份文件在当前设备上占用的总空间。',
	'Edit VRAM allocation': '编辑显存分配',
	'Unbind from this GPU': '解除与此 GPU 的绑定',
	"This will remove the app's access to this GPU and release its allocated resources":
		'移除后，该应用将无法继续使用此 GPU，并释放已分配的资源。',
	'Remove app from this GPU?': '将 {app} 从该 GPU 移除？',
	'Switch this app to another GPU': '为当前应用切换 GPU',
	'Switch {app} to another GPU': '为 {app} 切换 GPU',
	'This will switch “{app}” from the current GPU to {tgpu}':
		'“{app}”将从当前 GPU 切换到 {tgpu}',
	...
	'Only accounts belonging to the domain {domain} can be created.':
		'仅允许创建属于域 {domain} 的账户。',
	'No failed file': '无失败文件',
	'Failed File List': '失败文件列表',
	'Failed to create full-text index for some files.':
		'部分文件的全文索引创建失败',
	'Click to view the list': '点击查看列表',
	Gallery: '图库',
	...
	'Manage shared settings for your apps.': '管理应用的共享设置。',
	...
	Hide: '隐藏',
	Mirror: '镜像站',
	'Are you sure to reset password for': '您确定要重置"{olaresName}"的密码吗?',
	'Add environment variable': '添加环境变量',
	Type: '类型',
	'Access policies': '访问策略',
	'Manage sub policies': '管理子策略',
	'Endpoint settings': '端点配置',
	'Sub policies': '子策略',
	'Fine grained access control': '细粒度访问控制',
	'Set access policies for specific URLs': '为特定URL设置访问策略',
	'Add sub policy': '添加子策略',
	'SMB account management': 'SMB账号管理',
	'Manage your SMB network storage connections': '管理您的SMB网络存储连接',
	Protocol: '协议',
	Destination: '目标地址',
	'No SMB accounts': '暂无SMB账号',
	'Add an account to easily back up, restore, and sync your important data.':
		'添加账号即可轻松备份、恢复并同步您的重要数据。',
	'Add account': '添加账号',
	'Delete environment variable': '删除环境变量',
	'Are you sure to delete the environment variable? It cannot be recovered after deletion.':
		'确认要删除环境变量 "{envName}" 吗？删除后不可恢复。',
	Save: '保存',
	'Sign out': '登出',
	'Type an app name': '输入应用名称',
	'Low resources allocation': '资源分配不足',
	'The resources allocation is low. We recommend at least 1 CPU core and 3 GB of RAM. Lower settings may lead to poor experience. Are you sure you want to proceed?':
		'当前资源分配过低。建议至少配置 1 个 CPU 核心和 3 GB 内存。更低配置可能导致体验不佳。确定要继续吗？',
	Acknowledgements: '致谢',
	'Sincere thanks to our backer': '诚挚感谢我们的 {number} 位支持者',
	'Search backer ID or name...': '搜索支持者 ID 或姓名...',
	backer_name_not_found: '哎呀！没看到你的名字？请通过邮箱 {email} 联系我们！',
	'Stop the app before removing it from this GPU':
		'请先暂停应用，再将其从 GPU 移除。<br/>你可以在应用市场 > 我的 Olares 或设置 > 应用中暂停该应用。',
	'Minimum video memory required to allocate {min}': '最小需要分配{min}的显存',
	'Reset SSH Password': '重置 SSH 密码',
	Widget: '小组件',
	'Date & Time': '日期和时间',
	'24-hour format': '24小时制',
	'Date format': '日期格式',
	'Show Dashboard': '展示仪表盘',
	'Select an app to use this GPU exclusively': '选择要独占此 GPU 的应用',
	'Select GPU': '选择 GPU'
};
