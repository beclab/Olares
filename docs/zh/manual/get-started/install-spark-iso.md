...
1. 下载[适用于 Spark 的官方 Olares ISO 镜像](https://cdn.olares.com/spark/olares.iso)。
...
4. 按 **F10** 保存并重启。系统将自动进入 Olares 安装界面。
...
4. 在**编辑显存分配**对话框中，输入所需的显存大小（GB）并点击**确认**。
