...
2. 卸载现有的 AI 模型服务应用。例如：
    ...
    - 应用商店中预打包的模型应用
    - Ollama（作为基础引擎安装，模型需手动拉取）
    - ComfyUI

3. 从应用商店安装这些应用的新版本：

    - 对于预打包的模型应用和基于 Ollama 的模型：使用新的引擎基座应用（llama.cpp 引擎基座、Ollama 引擎基座、SGLang 引擎基座和 vLLM 引擎基座）部署所需模型。模型文件将自动保存到  `huggingface` 或 `ollama` 子目录中。
    - 对于 ComfyUI，重新下载所需的模型文件。它们将被自动保存到 `comfyui` 子目录中。
