...
// `model local gpu`  — GET  /api/diag/gpu
// `model local perf` — POST /api/diag/perf, GET /api/diag/perf/last
//
...
Examples:
  olares-cli model local gpu llamacppqwen3627bggufv3
  olares-cli model local gpu llamacppqwen3627bggufv3 --engine-native -o json
`,
...
func renderGPUReport(w io.Writer, li *llmInit, r *gpuReport) error {
	...
	if r.placement() == "not reported by this engine" {
		_, err := fmt.Fprintf(w, "\nThis engine reports no residency figures, so where the weights sit "+
			...
			"`olares-cli model local config %s`.\n", li.AppName)
		return err
	}
	return nil
}
...
Examples:
  olares-cli model local perf llamacppqwen3627bggufv3
  olares-cli model local perf llamacppqwen3627bggufv3 --last
  olares-cli model local perf llamacppqwen3627bggufv3 --decode-tokens 128 --with-think
`,
		Args: cobra.ExactArgs(1),
		RunE: func(c *cobra.Command, args []string) error {
			ctx := c.Context()
...
func diagErr(err error) error {
	...
	case re.Status == 404:
		return fmt.Errorf("%w\nThis application does not serve the diagnostic routes; "+
			"`olares-cli model local endpoints <app>` lists what it does serve", err)
	case re.Status == 503:
		return fmt.Errorf("%w\nThe engine did not answer its own introspection probe, which usually means "+
			"it is not up yet; `olares-cli model local status <app>` says which phase it is in", err)
	}
	return err
}
...
func perfRunErr(err error) error {
	...
	case "engine_not_ready":
		return fmt.Errorf("%w\nMeasuring an engine that is still starting would record timings for work it "+
			"is not doing yet; `olares-cli model local progress <app> --watch` follows it to ready", err)
...
Examples:
  olares-cli model local retry llamacppqwen3627bggufv3
  olares-cli model local retry llamacppqwen3627bggufv3 --level sha256
  olares-cli model local retry llamacppqwen3627bggufv3 --force
`,
		Args: cobra.ExactArgs(1),
		RunE: func(c *cobra.Command, args []string) error {
			ctx := c.Context()
			...
			_, err = fmt.Printf("%s: %s, from phase %s.\n`olares-cli model local progress %s --watch` follows it.\n",
...
"model local spec set", the changes an engine reads at launch — its mode, and
...
Examples:
  olares-cli model local restart llamacppqwen3627bggufv3
  olares-cli model local restart llamacppqwen3627bggufv3 --yes
`,
		Args: cobra.ExactArgs(1),
		RunE: func(c *cobra.Command, args []string) error {
			ctx := c.Context()
			...
			_, err = fmt.Printf("%s: the engine was told to relaunch. It is unavailable until the weights "+
				"are loaded again — `olares-cli model local status %s` says when.\n", li.AppName, li.AppName)
			return err
		},
	}
