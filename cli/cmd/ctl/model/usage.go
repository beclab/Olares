...
// `olares-cli model usage …` — what has been called, and what it cost.
//
...
Examples:
  olares-cli model usage summary --by model --since 7d
  olares-cli model usage summary --by day --since 30d
  olares-cli model usage summary --by user --status failed
`,
		Args: cobra.NoArgs,
...
Examples:
  olares-cli model usage list --since 1h
  olares-cli model usage list --status failed --limit 20
  olares-cli model usage list --key ci -o json
`,
		Args: cobra.NoArgs,
		RunE: func(c *cobra.Command, _ []string) error {
...
Examples:
  olares-cli model usage export --since 30d --out usage.csv
  olares-cli model usage export --status failed | head
`,
		Args: cobra.NoArgs,
		RunE: func(c *cobra.Command, _ []string) error {
