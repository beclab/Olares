<template>
	<q-select
		...
		class="custom-selection-wrapper"
		:disable="!$attrs?.options?.length"
	>
		...
		<template v-for="(slot, name) in $slots" v-slot:[name]="scope" :key="name">
...
::v-deep(
		.custom-selection-wrapper.q-field--highlighted .q-field__control:after
	) {
	border-width: 1px;
}
</style>
