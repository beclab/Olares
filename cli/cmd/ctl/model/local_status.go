...
// `model local status` — GET /healthz, /api/progress, /api/build-info
// `model local progress` — GET /api/progress
//
...
Phases mean different things to a caller:
  ...
  failed     it stopped trying; "model local retry" restarts the loop
...
Examples:
  ...
  olares-cli model local status "Qwen3.6-27B (llama.cpp)"
  olares-cli model local status llamacppqwen3627bggufv3 -o json
`,
...
func renderLocalStatus(w io.Writer, r *localStatusReport) error {
	...
	if r.Health != nil && !r.Health.Ready {
		_, err := fmt.Fprintln(w, "\nThis model refuses calls until it is ready. "+
			"`olares-cli model local progress "+r.App.AppName+" --watch` follows it.")
		return err
	}
	return nil
}
...
Examples:
  olares-cli model local progress llamacppqwen3627bggufv3
  olares-cli model local progress llamacppqwen3627bggufv3 --watch
  olares-cli model local progress llamacppqwen3627bggufv3 --watch --interval 5s
`,
		Args: cobra.ExactArgs(1),
		RunE: func(c *cobra.Command, args []string) error {
