...
func (r *FileserverReconciler) reconcile(ctx context.Context) error {
	...
	if err := r.cache.List(ctx, &userList); err != nil {
		return fmt.Errorf("list user failed: %w", err)
	}

	for _, user := range userList.Items {
		serviceNamespace := fmt.Sprintf("user-system-%s", user.Name)
		...
		for nodeName, pod := range podMap {
			...
			if err := fsCfg.upsertRole(ctx, r.client, user.Name); err != nil {
				return fmt.Errorf("upsert cluster role for %s: %w", nodeName, err)
			}
			if err := fsCfg.upsertRoleBinding(ctx, r.client, user.Name); err != nil {
...
func (f *fileServerProviderConfig) roleName(username string) string {
	return fmt.Sprintf("%s:files-frontend-domain-%s", username, f.NodeName)
}

func (f *fileServerProviderConfig) roleBindingName(username string) string {
	return fmt.Sprintf("user:%s:files-frontend-domain-%s", username, f.NodeName)
}
...
func (f *fileServerProviderConfig) upsertRole(ctx context.Context, c client.Client, username string) error {
	var role rbacv1.ClusterRole
	key := types.NamespacedName{Name: f.roleName(username)}

	err := c.Get(ctx, key, &role)
	if err != nil {
		if !apierrors.IsNotFound(err) {
			return fmt.Errorf("get cluster role %s: %w", f.roleName(username), err)
		}

		role = rbacv1.ClusterRole{
			ObjectMeta: metav1.ObjectMeta{
				Name: f.roleName(username),
				Annotations: map[string]string{
...
func (f *fileServerProviderConfig) upsertRoleBinding(ctx context.Context, c client.Client, username string) error {
	var binding rbacv1.ClusterRoleBinding
	key := types.NamespacedName{Name: f.roleBindingName(username)}

	err := c.Get(ctx, key, &binding)
	if err != nil {
		if !apierrors.IsNotFound(err) {
			return fmt.Errorf("get cluster role binding %s: %w", f.roleBindingName(username), err)
		}

		binding = rbacv1.ClusterRoleBinding{
			ObjectMeta: metav1.ObjectMeta{
				Name: f.roleBindingName(username),
			},
