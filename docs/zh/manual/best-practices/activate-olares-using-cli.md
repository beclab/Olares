...
使用本高级设置指南，从主机终端安装并激活全新或未初始化的 Olares 设备，例如 Olares One。
...
3. 当提示输入 Olares ID 时，输入你在 LarePass 应用中已注册的 ID。例如，`alice2026`。
...
2. 查看输出中的版本号，选择相应方式。
    ...
    v1.12.6 及更高版本的系统内置了具备激活功能的 CLI，无需额外下载。直接跳到步骤 3。
    </template>
...
1. 根据以下必需参数，准备激活命令：
    ...
    | `password` | 步骤 1 中生成的 Olares 默认登录密码。 |
    | `reset-password` | 用于替换默认密码的新登录密码。 |
    | `authurl` | 步骤 1 中的 Wizard URL。 |
    | `vault` | 步骤 1 中的 Wizard URL，后接 `/server`。 |
    | `bfl` | 步骤 1 中的 Wizard URL。 |
    | `host` | 步骤 3 中选择的 FRP 主机地址。 |
