...
Control the per-node task queue that backs the asynchronous file verbs (`compress` / `extract`). They return a `task_id`; this is how you act on it AFTER it's queued. (`files task` itself has no version floor; the `compress` / `extract` verbs that enqueue work do — see [olares-files-compress.md](olares-files-compress.md).)
