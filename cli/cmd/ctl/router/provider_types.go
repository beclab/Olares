...
// `olares-cli router provider types [vendor|type]`
//
...
Examples:
  olares-cli router provider types
  olares-cli router provider types anthropic
  olares-cli router provider types openai --models
`,
...
func runProviderTypes(ctx context.Context, f *cmdutil.Factory, ref string, withModels bool, outputRaw string) error {
	...
	if withModels && ref == "" {
		return fmt.Errorf("--models needs a vendor or provider type; run `olares-cli router provider types` to see them")
	}
