...
If you installed ComfyUI on Olares 1.12.5 or earlier, the file locations are different:
`/Files/External/<your-hostname>/ai/comfyui/` for the app, `/Files/External/<your-hostname>/ai/model/` for models, and `/Files/External/<your-hostname>/ai/output/comfyui/` for outputs.
After migrating to the new version, use the new paths above.
:::

In ComfyUI v3 (installed via Olares 1.12.6 or later), models are stored in `/Common/comfyui/model` and shared across AI apps. During migration from an earlier version, ComfyUI automatically copies models from the legacy `External/<your-hostname>/ai/model/` to the new `/Common/comfyui/model/` location.
...
ComfyUI uses a configuration file called `extra_model_paths.yaml` to locate models, custom nodes, and other resources outside the default ComfyUI directory. In Olares, this file is automatically generated and configured to point to the shared model directory, so you do not normally need to edit it.

The default configuration includes:

- **`base_path`**: Points to `/Common/comfyui/model` — the shared model hub where models are stored and shared across AI apps.

#### When you might need to customize it
...
:::warning
Incorrect changes to `extra_model_paths.yaml` may prevent ComfyUI from starting. Only edit this file if you are familiar with YAML syntax and understand the paths you are adding.
:::
