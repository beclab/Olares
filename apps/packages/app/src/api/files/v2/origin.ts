...
export default abstract class Origin {
	 ...
	 */
	abstract getPreviewURL(res: any, thumb: 'big' | 'thumb'): string;
	 ...
	 * getDiskPath
	 */
	abstract getDiskPath(selectFiles: any, type?: CommonUrlApiType): string;
	...
	abstract pathToFrontendFile(path: string): {
		isDir: boolean;
		fileExtend: string;
		path: string;
	};

	abstract uploadEmptyFile(tranfeItemId: number): Promise<any>;
}
