---
outline: deep
description: 了解如何启用和配置 OpenClaw 沙箱，以实现安全的代码执行。
head:
  - - meta
    - name: keywords
      content: Olares, OpenClaw, 沙箱, 安全, 代码执行
app_version: "1.0.2"
doc_version: "1.1"
...
为了最大程度地提升安全性并隔离潜在的危险操作，你可以启用 OpenClaw 沙箱。沙箱提供了一个专门用于代码执行的隔离的、临时的环境，确保你的核心系统始终受到保护。
...
配置沙箱时，`mode` 设置指定了沙箱的触发时机：
- **off**：沙箱已禁用。所有命令都在主容器中运行。
- **non-main**：沙箱会隔离通过 Discord 等外部渠道执行的命令。在 Control UI 的 **Chat** 页面中直接执行的命令将绕过沙箱，在主容器中运行。
- **all**：无论使用哪个界面或渠道，所有命令都在沙箱内运行。

## 启用沙箱

OpenClaw 沙箱默认处于禁用状态。你可以通过修改配置文件或使用 Control UI 来启用它。
...
4. 找到 `agents` > `defaults` 部分，然后将以下 `sandbox` 配置块添加进去。
    ...
    :::info 新安装用户与升级用户的区别
    <ul><li>如果你是全新安装，沙箱配置块已存在，只需将 <code>mode</code> 的值从 <code>off</code> 改为 <code>non-main</code>。</li><li>如果你是从旧版本升级，则需要粘贴整个配置块。</li></ul>
    :::
    ...
    ![通过配置文件启用沙箱](/images/zh/manual/use-cases/openclaw-edit-config-file.png#bordered)
...
4. 按如下方式配置设置：
    ...
    ![在 Control UI 中启用沙箱](/images/manual/use-cases/openclaw-sandbox-enable-ui1.png#bordered)
...
启用后，每当需要执行命令时，OpenClaw 会自动创建并使用隔离的沙箱环境。

要验证沙箱是否正常工作，请使用 Discord 等外部渠道进行测试：

1. 确保你已[集成 Discord](/use-cases/openclaw-integration.md)。
2. 在 Discord 中，向智能体发送以下私信：
    ...
    ```
3. 智能体会启动隔离的沙箱，安全地克隆仓库、读取文件并返回摘要。
4. 在智能体运行期间，打开 OpenClaw CLI 并运行以下命令以验证活动的沙箱：

    ```bash
    openclaw sandbox list
    ```

    终端将显示当前运行的沙箱容器，确认隔离已生效。

    ![验证沙箱](/images/manual/use-cases/openclaw-sandbox-verify.png#bordered)

## 授予额外的目录访问权限

默认情况下，`workspaceAccess: "rw"` 设置仅允许沙箱访问 OpenClaw 自身的工作区，以便智能体更新其记忆文件。

如果你希望沙箱与 Olares 文件交互，则必须使用自定义绑定挂载显式授予访问权限。这会将特定目录直接挂载到临时沙箱容器中。

### 授予访问权限

例如，要授予沙箱对 **Home** 目录的只读（`ro`）访问权限：
...
6. 按如下方式添加 `binds` 和 `dangerouslyAllowExternalBindSources` 行。确保在前面的 `"user": "1000:1000"` 行后面添加了逗号，以保持 JSON 语法有效。
    ...
    "dangerouslyAllowExternalBindSources": true //允许沙箱访问默认工作区之外的目录
    ```
...
在上一步中，沙箱模式为 `non-main`，绑定挂载设置为 `ro`。要了解这些设置如何协同工作，你可以通过两个不同的界面进行测试。
...
**原因**：通过 Control UI 的 **Chat** 页面发送的命令属于"主"会话。由于你将沙箱模式设置为 `non-main`，该会话完全绕过沙箱。智能体使用 OpenClaw 的默认系统权限写入文件。
...
**原因**：通过 Discord 等外部渠道发送的命令会触发沙箱。由于你为 **Home** 目录配置了只读（`ro`）绑定挂载，智能体被阻止写入或修改任何文件。

## 了解更多

- [OpenClaw 沙箱隔离文档](https://docs.openclaw.ai/zh-CN/gateway/sandboxing)
