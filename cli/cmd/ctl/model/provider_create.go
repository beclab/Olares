...
// `olares-cli model provider create` — POST /console/api/providers
// `olares-cli model provider register <app>` — the same route, source=olares
//
...
Examples:
  olares-cli model provider create --name openai-prod --type openai \
    --base-url https://api.openai.com/v1 --credential openai_api_key=sk-...

  printf '{"api_key":"%s"}' "$ANTHROPIC_KEY" |
    olares-cli model provider create --name claude --type anthropic \
...
func runProviderCreate(ctx context.Context, f *cmdutil.Factory, req createProviderRequest, credPairs []string, credJSON string, andValidate bool, outputRaw string) error {
	...
	if len(missing) > 0 {
		return fmt.Errorf("provider create needs %s; `olares-cli model provider types` lists the types and their credential fields",
	...
	if !andValidate {
		_, err := fmt.Fprintf(os.Stdout,
			"\ncreated with no models attached. Next: `olares-cli model provider validate %s`, "+
				"then choose models with `olares-cli model provider models add %s ...`\n",
...
Example:
  olares-cli model provider register qwen3-6-27b
`,
